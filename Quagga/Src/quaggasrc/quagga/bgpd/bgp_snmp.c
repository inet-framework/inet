...
peer_lookup_addr_ipv4 (struct in_addr *src)
{
  ...
  LIST_LOOP (bgp->peer, peer, nn)
    {
      ret = inet_pton (AF_INET, peer->host__item, &addr);
      if (ret > 0)
	{
...
bgp_peer_lookup_next (struct in_addr *src)
{
  ...
  LIST_LOOP (bgp->peer, peer, nn)
    {
      ret = inet_pton (AF_INET, peer->host__item, &su.sin.sin_addr);
      if (ret > 0)
	{
	  p = &su.sin.sin_addr;
...
bgpTrapEstablished (struct peer *peer)
{
  ...
  ret = inet_aton (peer->host__item, &addr);
  if (ret == 0)
    return;
