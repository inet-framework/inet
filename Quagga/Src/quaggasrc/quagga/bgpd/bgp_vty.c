...
extern struct in_addr router_id_zebra_bgpd;
  ...
  switch (error)
    ...
    case BGP_ERR_AF_UNCONFIGURED:
      vty_out (vty,
	       ...
	       peer->host__item, VTY_NEWLINE);
      break;
    case BGP_ERR_SOFT_RECONFIG_UNCONFIGURED:
      vty_out (vty, "%%BGP: Inbound soft reconfig for %s not possible as it%s      has neither refresh capability, nor inbound soft reconfig%s", peer->host__item, VTY_NEWLINE, VTY_NEWLINE);
      break;
    default:
      break;
    }
...
bgp_show_summary (struct vty *vty, struct bgp *bgp, int afi, int safi)
{
  ...
  LIST_LOOP (bgp->peer, peer, nn)
      ...
      if (peer->afc[afi][safi])
	{
	  ...
	  len = vty_out (vty, "%s", peer->host__item);
	  len = 16 - len;
	  if (len < 1)
...
bgp_show_peer_afi (struct vty *vty, struct peer *p, afi_t afi, safi_t safi)
{
  ...
  sprintf (orf_pfx_name, "%s.%d.%d", p->host__item, afi, safi);
...
bgp_show_peer (struct vty *vty, struct peer *p)
{
  ...
  vty_out (vty, "BGP neighbor is %s, ", p->host__item);
        ...
        afi_t afi, safi_t safi)
{
  ...
  len = vty_out (vty, "%s", rsclient->host__item);
  len = 16 - len;
...
struct cmd_node bgp_ipv4_unicast_node_bgpd =
{
  BGP_IPV4_NODE,
  "%s(config-router-af)# ",
  1,
...
struct cmd_node bgp_ipv4_multicast_node_bgpd =
{
  BGP_IPV4M_NODE,
  "%s(config-router-af)# ",
  1,
...
struct cmd_node bgp_ipv6_unicast_node_bgpd = 
{
  BGP_IPV6_NODE,
  "%s(config-router-af)# ",
  1,
...
struct cmd_node bgp_vpnv4_node_bgpd =
{
  BGP_VPNV4_NODE,
  "%s(config-router-af)# ",
  1
...
struct cmd_node community_list_node_bgpd =
{
  COMMUNITY_LIST_NODE,
  "",
  1				/* Export to vtysh.  */
