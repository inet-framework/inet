...
bgp_connect_check (struct peer *peer)
{
  ...
  BGP_WRITE_OFF (peer->t_write__item);
  ...
  if (status == 0)
  ...
  else
    {
	  ...
	  plog_debug (peer->log, "%s [Event] Connect failed (%s)",
		     peer->host__item, safe_strerror (errno));
  ...
  if (! stream_empty (s))
    {
      ...
      BGP_WRITE_ON (peer->t_write__item, bgp_write, peer->fd);
      stream_reset (s);
      return packet;
    }
  return NULL;
  ...
  if (BGP_DEBUG (update, UPDATE_OUT))
    {
      ...
      zlog (peer->log, LOG_DEBUG, "%s send UPDATE %s/%d %s",
	    peer->host__item, inet_ntop(p.family, &(p.u.prefix), buf, BUFSIZ),
	    p.prefixlen, attrstr);
    }
...
bgp_keepalive_send (struct peer *peer)
{
  ...
  if (BGP_DEBUG (keepalive, KEEPALIVE))  
    zlog_debug ("%s sending KEEPALIVE", peer->host__item); 
  if (BGP_DEBUG (normal, NORMAL))
    zlog_debug ("%s send message type %d, length (incl. header) %d",
               peer->host__item, BGP_MSG_KEEPALIVE, length);
...
bgp_open_send (struct peer *peer)
{
  ...
  if (BGP_DEBUG (normal, NORMAL))
    zlog_debug ("%s sending OPEN, version %d, my as %d, holdtime %d, id %s", 
	       peer->host__item, BGP_VERSION_4, local_as,
	       send_holdtime, inet_ntoa (peer->local_id));

  if (BGP_DEBUG (normal, NORMAL))
    zlog_debug ("%s send message type %d, length (incl. header) %d",
	       peer->host__item, BGP_MSG_OPEN, length);
			   ...
			   u_char *data, size_t datalen)
{
  ...
  if (BGP_DEBUG (normal, NORMAL))
    zlog_debug ("%s send message type %d, length (incl. header) %d",
	       peer->host__item, BGP_MSG_NOTIFY, length);
			...
			u_char orf_type, u_char when_to_refresh, int remove)
{
  ...
  if (BGP_DEBUG (normal, NORMAL))
	...
	zlog_debug ("%s sending REFRESH_REQ for afi/safi: %d/%d", 
		   peer->host__item, afi, safi);
      zlog_debug ("%s send message type %d, length (incl. header) %d",
		 peer->host__item, CHECK_FLAG (peer->cap, PEER_CAP_REFRESH_NEW_RCV) ?
		     ...
		     int capability_code, int action)
{
  ...
  if (capability_code == CAPABILITY_CODE_MP)
    {
      ...
      if (BGP_DEBUG (normal, NORMAL))
        zlog_debug ("%s sending CAPABILITY has %s MP_EXT CAP for afi/safi: %d/%d",
		   peer->host__item, action == CAPABILITY_ACTION_SET ?
  ...
  if (BGP_DEBUG (normal, NORMAL))
    zlog_debug ("%s send message type %d, length (incl. header) %d",
	       peer->host__item, BGP_MSG_CAPABILITY, length);
...
bgp_open_receive (struct peer *peer, bgp_size_t size)
{
  ...
  if (BGP_DEBUG (normal, NORMAL))
    zlog_debug ("%s rcv OPEN, version %d, remote-as %d, holdtime %d, id %s",
	       peer->host__item, version, remote_as, holdtime,
	       inet_ntoa (remote_id));
      ...
      if (! realpeer)
	{
	  ...
	  if (as)
		...
		zlog_debug ("%s bad OPEN, wrong router identifier %s",
			    peer->host__item, inet_ntoa (remote_id));
		...
		zlog_debug ("%s bad OPEN, remote AS is %d, expected %d",
			    peer->host__item, remote_as, peer->as);
	      bgp_notify_send_with_data (peer, BGP_NOTIFY_OPEN_ERR,
	  ...
	  && realpeer->status != OpenConfirm)
 	{
 	  if (BGP_DEBUG (events, EVENTS))
 	    zlog_debug ("%s [Event] peer's status is %s close connection",
		       realpeer->host__item, LOOKUP (bgp_status_msg, peer->status));
 	  return -1;
 	}
  ...
  if (version != BGP_VERSION_4)
	...
	zlog_debug ("%s bad protocol version, remote requested %d, local request %d",
		   peer->host__item, version, BGP_VERSION_4);
      bgp_notify_send_with_data (peer, 
				 BGP_NOTIFY_OPEN_ERR, 
...
bgp_update_receive (struct peer *peer, bgp_size_t size)
{
  ...
  if (peer->status != Established) 
    {
      zlog_err ("%s [FSM] Update packet received under status %s",
		peer->host__item, LOOKUP (bgp_status_msg, peer->status));
  ...
  if (withdraw_len > 0)
	...
	zlog_debug ("%s [Update:RECV] Unfeasible NLRI received", peer->host__item);
...
{
  if (BGP_DEBUG (keepalive, KEEPALIVE))  
    zlog_debug ("%s KEEPALIVE rcvd", peer->host__item); 
  ...
  if (size != BGP_MSG_ROUTE_REFRESH_MIN_SIZE - BGP_HEADER_SIZE)
    {
      ...
      if (size - (BGP_MSG_ROUTE_REFRESH_MIN_SIZE - BGP_HEADER_SIZE) < 5)
        {
          zlog_info ("%s ORF route refresh length error", peer->host__item);
      ...
      while (stream_pnt (s) < end)
	{
	  ...
	  if (orf_type == ORF_TYPE_PREFIX
	      ...
	      if (BGP_DEBUG (normal, NORMAL))
		{
		  zlog_debug ("%s rcvd Prefixlist ORF(%d) length %d",
			     peer->host__item, orf_type, orf_len);
		}

	      /* ORF prefix-list name */
	      sprintf (name, "%s.%d.%d", peer->host__item, afi, safi);
	      ...
	      while (p_pnt < p_end)
		{
		  ...
		  if (common & ORF_COMMON_PART_REMOVE_ALL)
		    {
		      if (BGP_DEBUG (normal, NORMAL))
			zlog_debug ("%s rcvd Remove-All pfxlist ORF request", peer->host__item);
		  ...
		  if (ret != CMD_SUCCESS)
		    {
		      if (BGP_DEBUG (normal, NORMAL))
			zlog_debug ("%s Received misformatted prefixlist ORF. Remove All pfxlist", peer->host__item);
		      prefix_bgp_orf_remove_all (name);
		      break;
		    }
		}
      ...
      if (BGP_DEBUG (normal, NORMAL))
	zlog_debug ("%s rcvd Refresh %s ORF request", peer->host__item,
		   when_to_refresh == REFRESH_DEFER ? "Defer" : "Immediate");
  ...
  while (pnt < end)
    {
      ...
      if (pnt + 3 > end)
        {
          zlog_info ("%s Capability length error", peer->host__item);
          bgp_notify_send (peer, BGP_NOTIFY_CEASE, 0);
          return -1;
        }
	...
	zlog_debug ("%s CAPABILITY has action: %d, code: %u, length %u",
		   peer->host__item, action, cap.code, cap.length);
...
bgp_capability_receive (struct peer *peer, bgp_size_t size)
{
  ...
  if (BGP_DEBUG (normal, NORMAL))
    zlog_debug ("%s rcv CAPABILITY", peer->host__item);
  ...
  if (peer->status != Established)
    {
      plog_err (peer->log,
		"%s [Error] Dynamic capability packet received under status %s", peer->host__item, LOOKUP (bgp_status_msg, peer->status));
      bgp_notify_send (peer, BGP_NOTIFY_FSM_ERR, 0);
      return;
    }
...
bgp_read (struct thread *thread)
{
  ...
  if (peer->ibuf->putp < BGP_HEADER_SIZE)
	...
	zlog_debug ("%s rcv message type %d, length (excl. header) %d",
		   peer->host__item, type, size - BGP_HEADER_SIZE);
  ...
  if (CHECK_FLAG (peer->sflags, PEER_STATUS_ACCEPT_PEER))
    {
      if (BGP_DEBUG (events, EVENTS))
	zlog_debug ("%s [Event] Accepting BGP peer delete", peer->host__item);
      peer_delete (peer);
    }
  return 0;
}
