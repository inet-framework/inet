...
void irdp_advert_off(struct interface_FOO *ifp);


char b1[16], b2[16], b3[16], b4[16];  /* For inet_2a */

struct prefix *irdp_get_prefix(struct interface_FOO *ifp)
{
...
int if_group (struct interface_FOO *ifp, 
	  int sock, 
	  u_int32_t group, 
	  int add_leave)
{
...
int if_add_group (struct interface_FOO *ifp)
{
...
int if_drop_group (struct interface_FOO *ifp)
{
  struct zebra_if *zi= ifp->info;
  struct irdp_interface *irdp = &zi->irdp;
  int ret;
...
struct interface_FOO *get_iflist_ifp(unsigned int idx)
{
  struct listnode *node;
...
void
if_set_defaults(struct interface_FOO *ifp)
{
  struct zebra_if *zi=ifp->info;
  struct irdp_interface *irdp=&zi->irdp;
...
void irdp_if_start(struct interface_FOO *ifp, int multicast, int set_defaults)
{
  struct zebra_if *zi= ifp->info;
  struct irdp_interface *irdp = &zi->irdp;
  struct listnode *node;
  ...
  irdp->t_advertise = thread_add_timer(zebrad.master__item, 
				       irdp_send_thread, 
				       ifp, 
				       timer);
}

void irdp_if_stop(struct interface_FOO *ifp)
{
  struct zebra_if *zi=ifp->info;
  struct irdp_interface *irdp=&zi->irdp;
...
void irdp_if_shutdown(struct interface_FOO *ifp)
{
  struct zebra_if *zi= ifp->info;
  struct irdp_interface *irdp = &zi->irdp;
...
void irdp_if_no_shutdown(struct interface_FOO *ifp)
{
  struct zebra_if *zi= ifp->info;
  struct irdp_interface *irdp = &zi->irdp;
...
void irdp_config_write (struct vty *vty, struct interface_FOO *ifp)
{
  struct zebra_if *zi=ifp->info;
  struct irdp_interface *irdp=&zi->irdp;
  struct Adv *adv;
