...
static struct prefix_master prefix_master_ipv4_lib = 
{ 
  {NULL, NULL},
  {NULL, NULL},
  1,
...
static struct prefix_master prefix_master_orf_lib = 
{ 
  {NULL, NULL},
  {NULL, NULL},
  1,
			  ...
			  int update_list)
{
  ...
  if (update_list)
    {
      if (plist->master__item->delete_hook)
	(*plist->master__item->delete_hook) (plist);

      if (plist->head == NULL && plist->tail == NULL && plist->desc == NULL)
	prefix_list_delete (plist);
      else
	plist->master__item->recent = plist;
    }
}
		       ...
		       struct prefix_list_entry *pentry)
{
  ...
  /* Run hook function. */
  if (plist->master__item->add_hook)
    (*plist->master__item->add_hook) (plist);
...
/* Prefix-list node. */
struct cmd_node prefix_node_lib =
{
  PREFIX_NODE,
