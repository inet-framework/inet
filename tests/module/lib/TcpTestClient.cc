...
class INET_API TcpTestClient : public cSimpleModule
{
  protected:
    struct Command
    {
        simtime_t tSend;
        int numBytes;
        bool eor = false; // MSG_EOR: optional "eor" keyword after numBytes in sendScript
        bool zerocopy = false; // MSG_ZEROCOPY: optional "zerocopy" keyword after numBytes in sendScript
    };
...
void TcpTestClient::parseScript(const char *script)
{
    ...
    while (*s)
    {
        ...
        // MSG_ZEROCOPY: optional "zerocopy" keyword right after the
