...
# CFM continuity checks, which INET cannot round-trip -- see SERIALIZER_REMAINING_GAPS.md)
...
- **MRA (auto-manager) MRP frames** were refused three times before they were admitted as
  `mrp-mra.pcap`; the story is in SERIALIZER_REMAINING_GAPS.md, and it is the best example
...
- **PPP** was refused as a capture of its own -- and then arrived anyway: the RTP example
  ...
  because the two line up. See SERIALIZER_REMAINING_GAPS.md.
