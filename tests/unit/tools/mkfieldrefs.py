...
tshark), and keeps a Wireshark field rename in this tool rather than in the repository.
    ...
    tests/unit/tools/mkfieldrefs.py pcap/bgp.pcap --frames 1
    tests/unit/tools/mkfieldrefs.py pcap/generated/mrp-ring.pcap        # every frame

Run it from tests/unit. The last column of every line names the tshark field the value
...
def main():
    ...
    lines = ["# Field reference for %s: what tshark reads from it, named as INET names it." % os.path.basename(args.pcap),
             "# Generated by tests/unit/tools/mkfieldrefs.py with %s -- do not edit by hand." % tshark_version(),
