...
|---|---|---|---|
| `serializer_pcap` | the 41 that round-trip + 4 derived + 7 generated | 4176 total, **4176 same** | byte-exactness is gated |
| `serializer_pcap_known_failures` | the 6 with gaps | 341 total, 303 same, **22 differ, 16 error** | the counts are pinned |
...
| IEEE 802.11 | 1 err | policy | an unsuccessful Reassociation Response carrying a **non-zero AID** is rejected | a decision on the AID policy | **S** |
| IPsec ESP | 6 err | not fixable | ESP **trailer is inside the ciphertext** | (needs decryption) | **—** |

The 22 differing frames are all in the MIPv6 / SCTP / DHCP captures; the 16 error frames
break down as **6 ESP + 9 IEEE 802.11** control frames of unmodelled subtypes **+ 1 IEEE
...
## IEEE 802.11 — a failed reassociation that still names an AID (1 error)

Frame 60 of `wlan-blockack-reassoc.pcap` is a Reassociation Response whose status is not
success but whose AID field still reads `0xc001`. The management serializer holds the AID
of an unsuccessful response to zero in both directions -- on purpose, so that a simulation
cannot put a stale association ID on the wire, and `Ieee80211MgmtFrameSerializer_1.test`
pins the rejection -- so the deserializer throws at byte offset 24 and the frame counts as
an error. It is cut out of `derived/wlan-blockack-reassoc-clean.pcap`.

**Fix:** a decision rather than a repair: either keep the policy and this entry, or let the
model carry whatever an unsuccessful response has in the field. **Effort S.**
...
`tests/unit/lib/fillers/`, one or more per type, a variant per branch the serializer takes
...
cd tests/unit/lib && make MODE=coverage KERNEL_LIBS=-loppsim_dbg \
...
for t in serializer_chunk_roundtrip serializer_pcap serializer_pcap_known_failures; do
  (cd tests/unit/work/$t && opp_makemake -f --deep -o $t -lINET_coverage -L../../../../src \
...
for t in serializer_chunk_roundtrip serializer_pcap serializer_pcap_known_failures; do
  (cd tests/unit/work/$t && LLVM_PROFILE_FILE=/tmp/cov-$t.profraw \
