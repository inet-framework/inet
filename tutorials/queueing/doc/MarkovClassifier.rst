...
.. In this step, packets are collected at random intervals by two active sinks
   (:ned:`ActivePacketSink`). One sink collects packets at a slower rate while the other
   sink uses a faster rate. The two packet sinks are combined using a Markov
   chain (:ned:`MarkovClassifier`) with a random transition matrix and random wait intervals. The packets are
