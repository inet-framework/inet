...
def countSentPackets(p, usePmtuValidator='false', n=1, t=0, r=-1):
	sentPackets = []
	
	for run in range(runs):
		file = path + 'msgLength='+msgLength+'/p='+str(p)+'/usePmtuValidator='+usePmtuValidator+'/r='+str(r)+'/n='+str(n)+'/t='+str(t)+'/'+str(run)+'.sca'
		#print(file)
...
for p in packetLossRates:
	...
	for r in reducePacketTimeThresholds:
		measurements[p][usePmtuValidator][r] = [[], []]
		...
		for n in lostPacketThresholds:
			for t in timeThresholds:						
				...
				sentPacketsMean,sentPacketsCi = mean_confidence_interval(countSentPackets(p, usePmtuValidator, n, t, r))
				measurements[p][usePmtuValidator][r][0].append(sentPacketsMean)
				measurements[p][usePmtuValidator][r][1].append(sentPacketsCi)

				print("m="+msgLength+", p="+str(p)+", r="+str(r)+", n="+str(n)+", t="+str(t)+": sentPacketsMean="+str(sentPacketsMean)+", diff="+str(sentPacketsMean-measurements[p]['false'][0]))
