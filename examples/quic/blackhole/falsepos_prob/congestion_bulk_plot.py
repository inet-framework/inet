...
result = np.load(path+"congestion_bulk.npy", allow_pickle=True).item()
...
nCounts = {}
for n in ns:
	nCounts[n] = (sum(x >= n for x in measurements[congestionDurationsShow][reducePacketTimeThresholdsShow][0][0]) / runs)
...
for t in ts:
	if t == 0.0:
		...
		tCounts[t] = (sum(x >= t for x in measurements[congestionDurationsShow][reducePacketTimeThresholdsShow][1][0]) / runs)
...
for n in ns:
	...
	for t in ts:
		...
		ax.bar(nIndex*len(ts)+tIndex, nxtValue, label='n='+str(n)+', t='+str(t*1000)+'ms')
		tIndex += 1
	nIndex += 1
	#print(nCounts[n])
...
fig.savefig('plots/congestion_bulk_'+str(reducePacketTimeThresholdsShow)+'_'+str(congestionDurationsShow)+'.png')
plt.show()
