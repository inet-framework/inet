...
result = np.load(path+"congestion_variable.npy", allow_pickle=True).item()
...
for pktPerSecsIndex in range(len(pktPerSecs)):
	for n in ns:
		nCounts[n].append(sum(x >= n for x in measurements[congestionDurationsShow][reducePacketTimeThresholdsShow][0][pktPerSecsIndex]) / runs)
...
for pktPerSecsIndex in range(len(pktPerSecs)):
	...
	for t in ts:
		if t == 0.0:
			...
			tCounts[t].append(sum(x >= t for x in measurements[congestionDurationsShow][reducePacketTimeThresholdsShow][1][pktPerSecsIndex]) / runs)
...
plt.title('conn time=10s, cd='+str(congestionDurationsShow)+'ms, reducePacketTimeThreshold=' + str(reducePacketTimeThresholdsShow))
...
fig.savefig('plots/congestion_variable_'+str(reducePacketTimeThresholdsShow)+'_'+str(congestionDurationsShow)+'.png')
plt.show()
