...
result = np.load(path+"congestion_fixed.npy", allow_pickle=True).item()
congestionDurations = result['congestionDurations']
...
for pktPerSecsIndex in range(len(pktPerSecs)):
	for n in ns:
		nCounts[n].append(sum(x >= n for x in measurements[congestionDurationsShow][reducePacketTimeThresholdsShow][msgLengthsShow][0][pktPerSecsIndex]) / runs)

#ts = [0, 0.002, 0.02, 0.2]
#ts = [0.0, 0.001, 0.050, 0.200]
#ts = [0.0, 0.020, 0.040, 0.060]
ts = [0.0, 0.020, 0.040, 0.060, 0.080]
tCounts = {}
for t in ts:
	tCounts[t] = []
for pktPerSecsIndex in range(len(pktPerSecs)):
	...
	for t in ts:
		if t == 0.0:
			...
			tCounts[t].append(sum(x >= t for x in measurements[congestionDurationsShow][reducePacketTimeThresholdsShow][msgLengthsShow][1][pktPerSecsIndex]) / runs)
...
plt.title('conn time=10s, cd='+str(congestionDurationsShow)+'ms, r=' + str(reducePacketTimeThresholdsShow) + ', msgLength='+msgLengthsShow)
...
fig.savefig('plots/congestion_fixed_'+str(reducePacketTimeThresholdsShow)+'_'+str(congestionDurationsShow)+'_'+msgLengthsShow+'.png')
plt.show()
