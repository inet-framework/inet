...
result = np.load(path+"lossylink_variable.npy", allow_pickle=True).item()
...
for pktPerSecsIndex in range(len(pktPerSecs)):
	for n in ns:
		nCounts[n].append(sum(x >= n for x in measurements[packetLossRatesShow][reducePacketTimeThresholdsShow][0][pktPerSecsIndex]) / runs)
...
for pktPerSecsIndex in range(len(pktPerSecs)):
	...
	for t in ts:
		if t == 0.0:
			...
			tCounts[t].append(sum(x >= t for x in measurements[packetLossRatesShow][reducePacketTimeThresholdsShow][1][pktPerSecsIndex]) / runs)
...
fig.savefig('plots/lossylink_variable_'+str(reducePacketTimeThresholdsShow)+'_'+str(packetLossRatesShow)+'.png')
plt.show()
