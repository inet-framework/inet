	...
	for reducePacketTimeThreshold in reducePacketTimeThresholds:
		...
		for pktInterval in pktIntervals:
			...
			for run in range(runs):
				file = path + 'lossylink_variable-pktInterval='+str(pktInterval)+',p='+str(packetLossRate)+',reducePacketTimeThreshold='+str(reducePacketTimeThreshold)+'-'+str(run)+'.vec'
				#print(file)
...
np.save(path + "lossylink_variable", result, allow_pickle=True)
