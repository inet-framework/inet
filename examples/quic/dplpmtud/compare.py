...
for alg in algs:
	for mtu in mtus:
		...
		file = path + "General-alg="+alg+",mtu="+str(mtu)+"-#0"
		#print(file)
	...
	from scalar 
	where moduleName = 'Simple.sender.quic'
	"""):
	...
	where vectorId = (
		select vectorId 
		from vector 
		where moduleName = 'Simple.sender.quic' and vectorName = 'dplpmtudSearchPmtu_cid=0_pid=0:vector'
	)
	order by simtimeRaw
	"""):
