...
for msgLength in msgLengths:
		...
		for lostPacketThreshold in lostPacketThresholds:
			...
			for timeThreshold in timeThresholds:
				...
				for pktPerSecsIndex in range(len(pktPerSecs)):
					diffs.append(measurements[msgLength]['false'][lostPacketThreshold][timeThreshold][pc][reducePacketTimeThreshold]['sentPackets'][0][pktPerSecsIndex] - measurements[msgLength]['true'][lostPacketThreshold][timeThreshold][pc][reducePacketTimeThreshold]['sentPackets'][0][pktPerSecsIndex])
					negError.append(-measurements[msgLength]['true'][lostPacketThreshold][timeThreshold][pc][reducePacketTimeThreshold]['sentPackets'][1][pktPerSecsIndex])
			
				#for ptb in ptbs:
				#	label = 'ptb='+ptb		
				#	ax[msgLengthIndex].errorbar(pktPerSecs, measurements[msgLength][ptb][lostPacketThreshold][timeThreshold][pc][reducePacketTimeThreshold]['sentPackets'][0], measurements[msgLength][ptb][lostPacketThreshold][timeThreshold][pc][reducePacketTimeThreshold]['sentPackets'][1], errorevery=1, label=label)
				ax[msgLengthIndex].fill_between(pktPerSecs, measurements[msgLength]['true'][lostPacketThreshold][timeThreshold][pc][reducePacketTimeThreshold]['sentPackets'][1], negError, color="lightgray", label="w/ PTB msg")
				ax[msgLengthIndex].errorbar(pktPerSecs, diffs, measurements[msgLength]['false'][lostPacketThreshold][timeThreshold][pc][reducePacketTimeThreshold]['sentPackets'][1], errorevery=1, label='w/o PTB msg')
	...
	ax[msgLengthIndex].set_ylabel('Add. Sent Packets')
...
fig.savefig(plot_dir+"decrease_app-limited_blowup_sentPackets.pdf")
plt.show()