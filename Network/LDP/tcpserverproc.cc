...
void TCPServerProc::activity()
{
	  ...
	  //msg_length = msg->par("num_bit_req");
       ev << "TCP_SERVER_PROC DEBUG:  LSR(" << IPAddress(local_addr).getString() << ") received SYN from LSR(" <<
	  ...
	  msgLng = tcp_mss/4;  //bits
	   ...
	   ev << "TCP_SERVER_PROC DEBUG:  LSR(" << IPAddress(local_addr).getString() << ") received ESTAB from LSR(" <<
	   IPAddress(rem_addr).getString() << ")\n";;
...
void TCPServerProc::sendKEEP_ALIVE()
{
	  ...
	  ev << "TCP_SERVER_PROC DEBUG: " << IPAddress(local_addr).getString() << " sends KEEP ALIVE to " <<
	  IPAddress(rem_addr).getString() << "\n";
...
void TCPServerProc::sendData(cMessage* payload)
{
  cMessage *send_call = new cMessage("TCP_C_SEND", TCP_C_SEND);
	  ...
	  ev << "TCP_SERVER_PROC DEBUG: " << IPAddress(local_addr).getString() << " sends data to " <<
	  IPAddress(rem_addr).getString() << "\n";
...
void TCPServerProc::processData(cMessage* msg)
{
    if ((msg->kind()) == TCP_I_SEG_FWD)
	{
		...
		{
			ev << "TCP_SERVER_PROC DEBUG: " << IPAddress(local_addr).getString() << " receives KEEP ALIVE message from " <<
			IPAddress(rem_addr).getString() << "\n";
			delete msg;
		}
  	}
   ...
   else
   {
      ...
      IPAddress(rem_addr).getString() << ", kind is " <<
      msg->kind() << "\n";
