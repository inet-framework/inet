...
static struct_ospf_master ospf_master_ospfd;

/* OSPF process wide configuration pointer to export. */
struct_ospf_master *om_ospfd;

extern struct_zclient *zclient_ospfd;
extern struct in_addr router_id_zebra_ospfd;
...
ospf_master_init ()
{
  memset (&ospf_master, 0, sizeof (struct_ospf_master));

  om = &ospf_master;
  om->ospf = list_new ();
  om->master__item = thread_master_create ();
