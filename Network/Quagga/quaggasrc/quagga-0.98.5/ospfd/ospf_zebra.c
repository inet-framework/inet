...
struct in_addr router_id_zebra_ospfd;

/* Router-id update message from zebra. */
int
#undef	zclient
ospf_router_id_update_zebra (int command, struct_zclient *zclient,
			     zebra_size_t length)
{
  struct ospf *ospf;
...
int
#undef	zclient
ospf_interface_add (int command, struct_zclient *zclient, zebra_size_t length)
{
...
int
#undef	zclient
ospf_interface_delete (int command, struct_zclient *zclient,
                       zebra_size_t length)
{
  struct interface_FOO *ifp;
  struct stream *s;
...
int
#undef	zclient
ospf_interface_state_up (int command, struct_zclient *zclient,
                         zebra_size_t length)
{
  struct interface_FOO *ifp;
  ...
  if (if_is_operative (ifp))
    {
      ...
      struct interface_FOO if_tmp;
      memcpy (&if_tmp, ifp, sizeof (struct interface_FOO));
...
int
#undef	zclient
ospf_interface_state_down (int command, struct_zclient *zclient,
                           zebra_size_t length)
{
  struct interface_FOO *ifp;
  struct ospf_interface *oi;
...
int
#undef	zclient
ospf_interface_address_add (int command, struct_zclient *zclient,
                            zebra_size_t length)
{
  struct ospf *ospf;
  struct connected *c;
...
int
#undef	zclient
ospf_interface_address_delete (int command, struct_zclient *zclient,
                               zebra_size_t length)
{
  struct ospf *ospf;
  struct connected *c;
...
int
#undef	zclient
ospf_zebra_read_ipv4 (int command, struct_zclient *zclient,
                      zebra_size_t length)
{
  struct stream *s;
  struct zapi_ipv4 api;
