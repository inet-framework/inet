...
void
ospf_if_recalculate_output_cost (struct interface_FOO *ifp)
{
  u_int32_t newcost;
...
/* Simulate down/up on the interface.  This is needed, for example, when 
   the MTU changes. */
void
ospf_if_reset(struct interface_FOO *ifp)
{
  struct route_node *rn;
...
ospf_if_table_lookup (struct interface_FOO *ifp, struct prefix *prefix)
{
  struct prefix p;
  struct route_node *rn;
...
void
ospf_add_to_if (struct interface_FOO *ifp, struct ospf_interface *oi)
{
  struct route_node *rn;
  struct prefix p;
...
void
ospf_delete_from_if (struct interface_FOO *ifp, struct ospf_interface *oi)
{
  struct route_node *rn;
  struct prefix p;
...
ospf_if_lookup_by_local_addr (struct ospf *ospf,
			      struct interface_FOO *ifp, struct in_addr address)
{
  struct listnode *node;
  struct ospf_interface *oi;
  ...
  if (oi->obuf)
    {
	 ...
	 listnode_delete (ospf->oi_write_q, oi);
         if (list_isempty(ospf->oi_write_q))
           OSPF_TIMER_OFF (ospf->t_write__item);
	 oi->on_write_q = 0;
       }
    }
}
...
void
ospf_free_if_params (struct interface_FOO *ifp, struct in_addr addr)
{
  struct ospf_if_params *oip;
...
struct ospf_if_params *
ospf_lookup_if_params (struct interface_FOO *ifp, struct in_addr addr)
{
  struct prefix_ipv4 p;
  struct route_node *rn;
...
struct ospf_if_params *
ospf_get_if_params (struct interface_FOO *ifp, struct in_addr addr)
{
  struct prefix_ipv4 p;
  struct route_node *rn;
...
void
ospf_if_update_params (struct interface_FOO *ifp, struct in_addr addr)
{
  struct route_node *rn;
  struct ospf_interface *oi;
...
int
ospf_if_new_hook (struct interface_FOO *ifp)
{
  int rc = 0;
...
int
ospf_if_delete_hook (struct interface_FOO *ifp)
{
  int rc = 0;
  struct route_node *rn;
#ifdef HAVE_OPAQUE_LSA
...
u_char
ospf_default_iftype(struct interface_FOO *ifp)
{
