...
void
lifreq_set_name (struct lifreq *lifreq, struct interface_FOO *ifp)
{
 ...
 */
void
if_get_metric (struct interface_FOO *ifp)
{
...
/* get interface MTU */
void
if_get_mtu (struct interface_FOO *ifp)
{
  struct lifreq lifreq;
  int ret;
...
int
if_set_prefix (struct interface_FOO *ifp, struct connected *ifc)
{
  int ret;
  struct ifreq ifreq;
...
/* Set up interface's address, netmask (and broadcast).
   Solaris uses ifname:number semantics to set IP address aliases. */
int
if_unset_prefix (struct interface_FOO *ifp, struct connected *ifc)
{
  int ret;
  struct ifreq ifreq;
  struct sockaddr_in addr;
...
/* get interface flags */
void
if_get_flags (struct interface_FOO *ifp)
{
  int ret;
  struct lifreq lifreq;
...
/* Set interface flags */
int
if_set_flags (struct interface_FOO *ifp, unsigned long flags)
{
  int ret;
  struct lifreq lifreq;
...
int
if_unset_flags (struct interface_FOO *ifp, unsigned long flags)
{
  int ret;
  struct lifreq lifreq;
...
int
if_prefix_add_ipv6 (struct interface_FOO *ifp, struct connected *ifc)
{
...
int
if_prefix_delete_ipv6 (struct interface_FOO *ifp, struct connected *ifc)
{
  char addrbuf[INET_ADDRSTRLEN];
