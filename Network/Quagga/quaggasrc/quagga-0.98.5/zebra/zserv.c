...
struct thread *t_write_zebra;
struct fifo message_queue_zebra;
...
zebra_server_dequeue (struct thread *t)
{
  ...
  if (FIFO_TOP (&message_queue))
    THREAD_WRITE_ON (zebrad.master__item, t_write, zebra_server_dequeue, 
                     NULL, sock);
		      ...
		      unsigned long written)
{
  ...
  THREAD_WRITE_ON (zebrad.master__item, t_write, zebra_server_dequeue, NULL, sock);
}
...
int
zsend_interface_delete (struct zserv *client, struct interface_FOO *ifp)
{
  struct stream *s;
...
zsend_interface_address (int cmd, struct zserv *client, 
                         struct interface_FOO *ifp, struct connected *ifc)
{
  int blen;
  struct stream *s;
  struct prefix *p;
...
zebra_client_close (struct zserv *client)
{
  ...
  if (client->t_write__item)
    thread_cancel (client->t_write__item);
...
zebra_event (enum event event, int sock, struct zserv *client)
{
  switch (event)
    {
    case ZEBRA_SERV:
      thread_add_read (zebrad.master__item, zebra_accept, client, sock);
      break;
    case ZEBRA_READ:
      client->t_read = 
	thread_add_read (zebrad.master__item, zebra_client_read, client, sock);
      break;
    case ZEBRA_WRITE:
      /**/
      break;
...
struct cmd_node table_node_zebra =
{
  TABLE_NODE,
...
/* table node for routing tables. */
struct cmd_node forwarding_node_zebra =
{
  FORWARDING_NODE,
  "",				/* This node has no interface. */
  1
