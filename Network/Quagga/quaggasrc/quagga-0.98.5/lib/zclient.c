...
#undef	zclient
  ...
  zclient = XMALLOC (MTYPE_ZCLIENT, sizeof (struct_zclient));
  memset (zclient, 0, sizeof (struct_zclient));
...
void
#undef	zclient
zclient_free (struct_zclient *zclient)
{
...
void
#undef	zclient
zclient_init (struct_zclient *zclient, int redist_default)
{
  int i;
...
void
#undef	zclient
zclient_stop (struct_zclient *zclient)
{
  if (zclient_debug)
...
void
#undef	zclient
zclient_reset (struct_zclient *zclient)
{
...
void
zebra_interface_if_set_value (struct stream *s, struct interface_FOO *ifp)
{
  /* Read interface's index. */
  ifp->ifindex = stream_getl (s);
  ifp->status = stream_getc (s);
...
void
#undef	zclient
zclient_redistribute (int command, struct_zclient *zclient, int type)
{
...
void
#undef	zclient
zclient_redistribute_default (int command, struct_zclient *zclient)
{
...

extern struct thread_master *master_lib;

static void
#undef	zclient
zclient_event (enum event event, struct_zclient *zclient)
{
  switch (event)
    {
