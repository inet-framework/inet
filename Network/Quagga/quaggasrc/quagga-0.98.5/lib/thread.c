...
thread_cancel (struct thread *thread)
{
  switch (thread->type)
    {
    case THREAD_READ:
      assert (FD_ISSET (thread->u.fd, &thread->master__item->readfd));
      FD_CLR (thread->u.fd, &thread->master__item->readfd);
      thread_list_delete (&thread->master__item->read, thread);
      break;
    case THREAD_WRITE:
      assert (FD_ISSET (thread->u.fd, &thread->master__item->writefd));
      FD_CLR (thread->u.fd, &thread->master__item->writefd);
      thread_list_delete (&thread->master__item->write, thread);
      break;
    case THREAD_TIMER:
      thread_list_delete (&thread->master__item->timer, thread);
      break;
    case THREAD_EVENT:
      thread_list_delete (&thread->master__item->event, thread);
      break;
    case THREAD_READY:
      thread_list_delete (&thread->master__item->ready, thread);
      break;
    default:
      break;
    }
  thread->type = THREAD_UNUSED;
  thread_add_unuse (thread->master__item, thread);
}
