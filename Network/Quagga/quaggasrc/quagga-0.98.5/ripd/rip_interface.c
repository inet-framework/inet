...
void rip_enable_apply (struct interface_FOO *);
void rip_passive_interface_apply (struct interface_FOO *);
int rip_if_down(struct interface_FOO *ifp);
...
vector rip_enable_interface_ripd;

/* RIP enabled interface table. */
struct route_table *rip_enable_network_ripd;

/* Vector to store passive-interface name. */
static int passive_default_ripd;	/* are we in passive-interface default mode? */
vector Vrip_passive_nondefault_ripd;

...
void
rip_request_interface_send (struct interface_FOO *ifp, u_char version)
{
...
void
rip_request_interface (struct interface_FOO *ifp)
{
  struct rip_interface *ri;
...
int
rip_multicast_join (struct interface_FOO *ifp, int sock)
{
  struct listnode *cnode;
...
void
rip_multicast_leave (struct interface_FOO *ifp, int sock)
{
  struct listnode *cnode;
...
int
rip_if_ipv4_address_check (struct interface_FOO *ifp)
{
  struct listnode *nn;
  struct connected *connected;
  int count = 0;
...
int
#undef	zclient
rip_interface_down (int command, struct_zclient *zclient, zebra_size_t length)
{
  struct interface_FOO *ifp;
  struct stream *s;
...
int
#undef	zclient
rip_interface_up (int command, struct_zclient *zclient, zebra_size_t length)
{
  struct interface_FOO *ifp;
...
int
#undef	zclient
rip_interface_add (int command, struct_zclient *zclient, zebra_size_t length)
{
  struct interface_FOO *ifp;
...
int
#undef	zclient
rip_interface_delete (int command, struct_zclient *zclient,
		      zebra_size_t length)
{
  struct interface_FOO *ifp;
  struct stream *s;
...
int
rip_if_down(struct interface_FOO *ifp)
{
  struct route_node *rp;
...
int
#undef	zclient
rip_interface_address_add (int command, struct_zclient *zclient,
			   zebra_size_t length)
{
...
int
#undef	zclient
rip_interface_address_delete (int command, struct_zclient *zclient,
			      zebra_size_t length)
{
  struct connected *ifc;
  struct prefix *p;
...
int
rip_enable_network_lookup_if (struct interface_FOO *ifp)
{
  struct listnode *nn;
  struct connected *connected;
  struct prefix_ipv4 address;
...
void
rip_connect_set (struct interface_FOO *ifp, int set)
{
  struct listnode *nn;
  struct connected *connected;
  struct prefix_ipv4 address;
...
void
rip_enable_apply (struct interface_FOO *ifp)
{
  int ret;
  struct rip_interface *ri = NULL;
...
void
rip_passive_interface_apply (struct interface_FOO *ifp)
{
  struct rip_interface *ri;
...
struct cmd_node interface_node_ripd =
{
  INTERFACE_NODE,
  "%s(config-if)# ",
  1,
...
int
rip_interface_new_hook (struct interface_FOO *ifp)
{
...
int
rip_interface_delete_hook (struct interface_FOO *ifp)
{
