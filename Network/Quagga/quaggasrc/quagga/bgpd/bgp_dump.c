...
struct stream *bgp_dump_obuf_bgpd;

/* BGP dump strucuture for 'dump bgp all' */
struct bgp_dump bgp_dump_all_bgpd;

/* BGP dump structure for 'dump bgp updates' */
struct bgp_dump bgp_dump_updates_bgpd;

/* BGP dump structure for 'dump bgp routes' */
struct bgp_dump bgp_dump_routes_bgpd;

/* Dump whole BGP table is very heavy process.  */
struct thread *t_bgp_dump_routes_bgpd;

...
/* BGP node structure. */
struct cmd_node bgp_dump_node_bgpd =
{
  DUMP_NODE,
  "",
  1
