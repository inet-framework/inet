...
struct thread *bgp_scan_thread_bgpd ;

/* BGP import thread */
struct thread *bgp_import_thread_bgpd ;

/* BGP scan interval. */
int bgp_scan_interval_bgpd;

/* BGP import interval. */
int bgp_import_interval_bgpd;

/* Route table for next-hop lookup cache. */
struct bgp_table *bgp_nexthop_cache_table_bgpd[AFI_MAX];
struct bgp_table *cache1_table_bgpd[AFI_MAX];
struct bgp_table *cache2_table_bgpd[AFI_MAX];

/* Route table for connected route. */
struct bgp_table *bgp_connected_table_bgpd[AFI_MAX];

/* BGP nexthop lookup query client. */
static struct_zclient *zlookup_bgpd ;

/* BGP process function. */
int bgp_process (struct bgp *, struct_bgp_node *, afi_t, safi_t);

