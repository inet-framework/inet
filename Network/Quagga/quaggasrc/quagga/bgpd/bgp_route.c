...
void
bgp_info_add (struct_bgp_node *rn, struct bgp_info *ri)
{
  struct bgp_info *top;
...
void
bgp_info_delete (struct_bgp_node *rn, struct bgp_info *ri)
{
  if (ri->next)
	      ...
	      )))
    {
      /* NEXT-HOP Unchanged. */
	   ...
	   || (peer_sort (peer) == BGP_PEER_EBGP
	       && bgp_multiaccess_check_v4 (attr->nexthop, peer->host__item) == 0))
    {
      /* Set IPv4 nexthop. */
      if (p->family == AF_INET)
	{
  ...
  if (aspath_loop_check (ri->attr->aspath, rsclient->as))
    {
      if (BGP_DEBUG (filter, FILTER))
        zlog (rsclient->log, LOG_DEBUG,
             "%s [Update:SEND] suppress announcement to peer AS %d is AS path.",
             rsclient->host__item, rsclient->as);
      return 0;
    }
#endif /* BGP_SEND_ASPATH_CHECK */
...
void
bgp_best_selection (struct bgp *bgp, struct_bgp_node *rn, struct bgp_info_pair *result)
{
...
bgp_process_announce_selected (struct peer *peer, struct bgp_info *selected,
        struct_bgp_node *rn, struct attr *attr, afi_t afi, safi_t safi)
    {
  struct prefix *p;
...
bgp_process_rsclient (struct bgp *bgp, struct peer *rsclient,
        struct_bgp_node *rn, afi_t afi, safi_t safi)
{
  struct prefix *p;
  struct bgp_info *new_select;
  struct bgp_info *old_select;
...
int
bgp_process_main (struct bgp *bgp, struct_bgp_node *rn, afi_t afi, safi_t safi)
    {
  struct prefix *p;
  struct bgp_info *new_select;
  struct bgp_info *old_select;
...
int
bgp_process (struct bgp *bgp, struct_bgp_node *rn, afi_t afi, safi_t safi)
{
  switch (rn->table->type)
    {
    case BGP_TABLE_MAIN:
  ...
  if (peer->pcount[afi][safi] > peer->pmax[afi][safi])
    {
      ...
      zlog (peer->log, LOG_INFO,
           ...
           peer->host__item, afi, peer->pcount[afi][safi], peer->pmax[afi][safi]);
...
void
bgp_rib_remove (struct_bgp_node *rn, struct bgp_info *ri, struct peer *peer,
		afi_t afi, safi_t safi)
{
...
void
bgp_rib_withdraw (struct_bgp_node *rn, struct bgp_info *ri, struct peer *peer,
      ...
      int sub_type, struct prefix_rd *prd, u_char *tag)
{
      ...
      if (attrhash_cmp (ri->attr, attr_new))
        {
          ...
          if (BGP_DEBUG (update, UPDATE_IN))
            zlog (peer->log, LOG_DEBUG,
                    ...
                    peer->host__item,
                    inet_ntop(p->family, &p->u.prefix, buf, SU_ADDRSTRLEN),
                    p->prefixlen, rsclient->host__item);
 ...
 filtered: 
  ...
  if (BGP_DEBUG (update, UPDATE_IN))
        ...
        peer->host__item,
        inet_ntop (p->family, &p->u.prefix, buf, SU_ADDRSTRLEN),
        p->prefixlen, rsclient->host__item, reason);
...
bgp_withdraw_rsclient (struct peer *rsclient, afi_t afi, safi_t safi,
  ...
  if (ri && ! CHECK_FLAG (ri->flags, BGP_INFO_HISTORY))
    ...
    zlog (peer->log, LOG_DEBUG,
          "%s Can't find the route %s/%d", peer->host__item,
          inet_ntop (p->family, &p->u.prefix, buf, SU_ADDRSTRLEN),
          p->prefixlen);
		    ...
		    struct bgp_info *binfo, int display, safi_t safi)
{
  ...
  len = vty_out (vty, "%s", binfo->peer->host__item);
  len = 17 - len;
  if (len < 1)
       ...
       "Display the prefixlist filter\n")
{
  ...
  sprintf (name, "%s.%d.%d", peer->host__item, AFI_IP, SAFI_UNICAST);
       ...
       "Display the prefixlist filter\n")
{
  ...
  if (strncmp (argv[0], "m", 1) == 0)
    {
      sprintf (name, "%s.%d.%d", peer->host__item, AFI_IP, SAFI_MULTICAST);
      count =  prefix_bgp_show_prefix_list (NULL, AFI_IP, name);
      if (count)
	{
       ...
       "Display the prefixlist filter\n")
{
  ...
  sprintf (name, "%s.%d.%d", peer->host__item, AFI_IP6, SAFI_UNICAST);
...
#endif /* HAVE_IPV6 */

struct bgp_table *bgp_distance_table_bgpd;
