...
static int ospf_mpls_te_new_if (struct interface_FOO *ifp);
static int ospf_mpls_te_del_if (struct interface_FOO *ifp);
...
static void ospf_mpls_te_config_write_if (struct vty *vty, struct interface_FOO *ifp);
...
ospf_mpls_te_init (void)
{
  ...
  OspfMplsTE.iflist__item = list_new ();
  OspfMplsTE.iflist__item->del = del_mpls_te_link;
...
{
  list_delete (OspfMplsTE.iflist__item);

  OspfMplsTE.iflist__item = NULL;
  OspfMplsTE.status = disabled;
...
lookup_oi_by_ifp (struct interface_FOO *ifp,
...
lookup_linkparams_by_ifp (struct interface_FOO *ifp)
{
  struct listnode *node;
  struct mpls_te_link *lp;

  LIST_LOOP (OspfMplsTE.iflist__item, lp, node)
    if (lp->ifp == ifp)
      return lp;
...
{
  struct interface_FOO *ifp = lp->ifp;
...
static int
ospf_mpls_te_new_if (struct interface_FOO *ifp)
{
  ...
  listnode_add (OspfMplsTE.iflist__item, new);
...
static int
ospf_mpls_te_del_if (struct interface_FOO *ifp)
{
  struct mpls_te_link *lp;
  int rc = -1;
...
ospf_mpls_te_lsa_originate (void *arg)
{
  ...
  for (node = listhead (OspfMplsTE.iflist__item); node; nextnode (node))
    {
...
static void
ospf_mpls_te_config_write_if (struct vty *vty, struct interface_FOO *ifp)
{
  struct mpls_te_link *lp;
...
DEFUN (mpls_te_link_metric,
       ...
       "Metric\n")
{
  struct interface_FOO *ifp = (struct interface_FOO *) vty->index;
  struct mpls_te_link *lp;
  u_int32_t value;
