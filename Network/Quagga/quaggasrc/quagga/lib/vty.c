...
vtysh_read (struct thread *thread)
{
  ...
  if (!vty->t_write__item && buffer_flush_available(vty->obuf, vty->fd))
...
vty_close (struct vty *vty)
{
  ...
  if (vty->t_read)
    thread_cancel (vty->t_read);
  if (vty->t_write__item)
    thread_cancel (vty->t_write__item);
  if (vty->t_timeout)
...
struct thread_master *master_lib;
...
vty_event (enum event event, int sock, struct vty *vty)
{
  ...
  switch (event)
    ...
    case VTYSH_READ:
      ...
      break;
    case VTYSH_WRITE:
      vty->t_write__item = thread_add_write (master, vtysh_write, vty, sock);
      break;
#endif /* VTYSH */
    case VTY_READ:
    ...
    case VTY_WRITE:
      if (! vty->t_write__item)
	vty->t_write__item = thread_add_write (master, vty_flush, vty, sock);
      break;
