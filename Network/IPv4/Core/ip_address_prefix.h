...
class IPAddressPrefix : public IPAddress {
  ...
  public:
	...
	IPAddressPrefix(int i,                           // The IP address as an int
					unsigned int lp = DEFAULT_IP_ADDRESS_PREFIX_LEN,
					const char *n = NULL);
	IPAddressPrefix(int i0, int i1, int i2, int i3,  // "i0.i1.i2.i3" format
					unsigned int lp = DEFAULT_IP_ADDRESS_PREFIX_LEN,
					const char *n = NULL);
	IPAddressPrefix(const char *t,                   // IP address as text
					unsigned int lp = DEFAULT_IP_ADDRESS_PREFIX_LEN,
					const char *n = NULL);
	IPAddressPrefix(IPAddress *ip,                   // As an object
					unsigned int lp = DEFAULT_IP_ADDRESS_PREFIX_LEN,
					const char *n = NULL);

	IPAddressPrefix(const IPAddressPrefix& obj);
	virtual ~IPAddressPrefix() { }
	virtual cObject *dup() const { return new IPAddressPrefix(*this); }
