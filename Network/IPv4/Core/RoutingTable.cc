...
//
// FIXME: this function is dirty... should be done differently!
//
InterfaceEntry *RoutingTable::getInterfaceByIndex(int index)
{
    ...
    if (index >= ifEntryCtr) {
        ...
        //ev <<"\ngetInterfaceByIndex(): Nonexistent interface asked\n";
...
int RoutingTable::interfaceAddressToNo(const IPAddress& addr)
{
    Enter_Method("interfaceAddressToNo()");
    ...
    if (dev) {
        ...
        if (e->type == REMOTE) {
            ...
            if (dev_nb != -1) {
                ...
                dev_nb = interfaceAddressToNo(*gw);
                if (dev_nb != -1) {
            ...
            dev_nb = interfaceAddressToNo(*(e->host));
            if (dev_nb != -1) {
                // The interface for the gateway address exists
                e->interfaceNo = dev_nb;
                e->interfaceName = intrface[dev_nb]->name;
