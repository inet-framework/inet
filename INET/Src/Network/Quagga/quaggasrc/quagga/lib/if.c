 ...
 */         
int
if_cmp_func (struct interface_FOO *ifp1, struct interface_FOO *ifp2)
{
  unsigned int l1, l2;
  long int x1, x2;
  char *p1, *p2;
...
if_new ()
{
  ...
  ifp = XMALLOC (MTYPE_IF, sizeof (struct interface_FOO));
  memset (ifp, 0, sizeof (struct interface_FOO));
  return ifp;
}
...
void
if_delete (struct interface_FOO *ifp)
{
...
void
if_add_hook (int type, int (*func)(struct interface_FOO *ifp))
{
  switch (type) {
  case IF_NEW_HOOK:
...
/* Does interface up ? */
int
if_is_up (struct interface_FOO *ifp)
{
...
int
if_is_running (struct interface_FOO *ifp)
{
...
int
if_is_operative (struct interface_FOO *ifp)
{
  return ((ifp->flags & IFF_UP) &&
...
int
if_is_loopback (struct interface_FOO *ifp)
{
...
int
if_is_broadcast (struct interface_FOO *ifp)
{
...
/* Does this interface support broadcast ? */
int
if_is_pointopoint (struct interface_FOO *ifp)
{
...
int
if_is_multicast (struct interface_FOO *ifp)
{
...
/* For debugging */
void
if_dump (struct interface_FOO *ifp)
{
  struct listnode *node;
...
struct connected *
connected_delete_by_prefix (struct interface_FOO *ifp, struct prefix *p)
{
  struct listnode *node;
...
/* Find the IPv4 address on our side that will be used when packets
   are sent to dst. */
struct connected *
connected_lookup_address (struct interface_FOO *ifp, struct in_addr dst)
{
  struct prefix addr;
  struct listnode *cnode;
  struct prefix *p;
...
struct connected *
connected_add_by_prefix (struct interface_FOO *ifp, struct prefix *p, 
...
struct route_table *ifaddr_ipv4_table_lib;
/* struct route_table *ifaddr_ipv6_table; */

void
ifaddr_ipv4_add (struct in_addr *ifaddr, struct interface_FOO *ifp)
{
...
void
ifaddr_ipv4_delete (struct in_addr *ifaddr, struct interface_FOO *ifp)
{
  struct route_node *rn;
  struct prefix_ipv4 p;
