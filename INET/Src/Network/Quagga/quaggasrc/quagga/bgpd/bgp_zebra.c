...
static struct_zclient *zclient_bgpd ;
struct in_addr router_id_zebra_bgpd;

/* Router-id update message from zebra. */
int
#undef	zclient
bgp_router_id_update (int command, struct_zclient *zclient, zebra_size_t length)
{
...
int
#undef	zclient
bgp_interface_add (int command, struct_zclient *zclient, zebra_size_t length)
{
...
int
#undef	zclient
bgp_interface_delete (int command, struct_zclient *zclient,
		      zebra_size_t length)
{
  struct stream *s;
  struct interface_FOO *ifp;
...
int
#undef	zclient
bgp_interface_up (int command, struct_zclient *zclient, zebra_size_t length)
{
  struct stream *s;
  struct interface_FOO *ifp;
  struct connected *c;
...
int
#undef	zclient
bgp_interface_down (int command, struct_zclient *zclient, zebra_size_t length)
{
  struct stream *s;
  struct interface_FOO *ifp;
  struct connected *c;
...
int
#undef	zclient
bgp_interface_address_add (int command, struct_zclient *zclient,
			   zebra_size_t length)
{
...
int
#undef	zclient
bgp_interface_address_delete (int command, struct_zclient *zclient,
			      zebra_size_t length)
{
  struct connected *ifc;
...
int
#undef	zclient
zebra_read_ipv4 (int command, struct_zclient *zclient, zebra_size_t length)
{
  struct stream *s;
...
/* Zebra route add and delete treatment. */
int
zebra_read_ipv6 (int command, struct_zclient *zclient, zebra_size_t length)
{
  struct stream *s;
...
int
if_get_ipv6_global (struct interface_FOO *ifp, struct in6_addr *addr)
{
  struct listnode *cnode;
  struct connected *connected;
  struct prefix *cp; 
...
int
if_get_ipv6_local (struct interface_FOO *ifp, struct in6_addr *addr)
{
  struct listnode *cnode;
  struct connected *connected;
  struct prefix *cp; 
		 ...
		 struct bgp_nexthop *nexthop, struct peer *peer)
{
  ...
  if (local->sa.sa_family == AF_INET6)
    {
      struct interface_FOO *direct = NULL;
