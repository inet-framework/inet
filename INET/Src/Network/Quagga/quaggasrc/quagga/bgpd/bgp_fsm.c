...
bgp_start_timer (struct thread *thread)
{
  ...
  if (BGP_DEBUG (fsm, FSM))
    zlog (peer->log, LOG_DEBUG,
	  "%s [FSM] Timer (start timer expire).", peer->host__item);
...
bgp_stop (struct peer *peer)
{
  ...
  if (peer->status == Established)
	...
	zlog_info ("%%ADJCHANGE: neighbor %s Down %s", peer->host__item,
  ...
  for (afi = AFI_IP ; afi < AFI_MAX ; afi++)
	...
	peer->orf_plist[afi][safi] = NULL;
        ...
        sprintf (orf_name, "%s.%d.%d", peer->host__item, afi, safi);
...
bgp_start (struct peer *peer)
{
  ...
  switch (status)
    ...
    case connect_error:
      if (BGP_DEBUG (fsm, FSM))
	plog_debug (peer->log, "%s [FSM] Connect error", peer->host__item);
      BGP_EVENT_ADD (peer, TCP_connection_open_failed);
      break;
...
{
  if (BGP_DEBUG (fsm, FSM))
    zlog (peer->log, LOG_DEBUG, "%s [FSM] Hold timer expire", peer->host__item);
  ...
  if (CHECK_FLAG (peer->sflags, PEER_STATUS_ACCEPT_PEER))
    {
      zlog_info ("%s [Event] Accepting BGP peer is deleted", peer->host__item);
      peer_delete (peer);
      return -1;
    }
...
bgp_establish (struct peer *peer)
{
  ...
  if (bgp_flag_check (peer->bgp, BGP_FLAG_LOG_NEIGHBOR_CHANGES))
    zlog_info ("%%ADJCHANGE: neighbor %s Up", peer->host__item);
...
{
  if (BGP_DEBUG (fsm, FSM))
    zlog (peer->log, LOG_DEBUG, "%s [FSM] bgp_ignore called", peer->host__item);
  return 0;
}

...
struct FOO6  FSM [BGP_STATUS_MAX - 1][BGP_EVENTS_MAX - 1] = 
{
  {
...
bgp_event (struct thread *thread)
{
  ...
  if (BGP_DEBUG (fsm, FSM))
    plog_debug (peer->log, "%s [FSM] %s (%s->%s)", peer->host__item, 
	       bgp_event_str[event],
