...
void bgp_node_delete (struct_bgp_node *);
...
bgp_node_create ()
{
  ...
  rn = (struct_bgp_node *) XMALLOC (MTYPE_BGP_NODE, sizeof (struct_bgp_node));
  memset (rn, 0, sizeof (struct_bgp_node));
  return rn;
}
...
/* Free route node. */
void
bgp_node_free (struct_bgp_node *node)
{
...
static void
set_link (struct_bgp_node *node, struct_bgp_node *new)
{
  int bit;
...
/* Lock node. */
struct_bgp_node *
bgp_lock_node (struct_bgp_node *node)
{
  node->lock++;
  return node;
}
...
/* Unlock node. */
void
bgp_unlock_node (struct_bgp_node *node)
{
  node->lock--;
...
void
bgp_node_delete (struct_bgp_node *node)
{
...
struct_bgp_node *
bgp_route_next (struct_bgp_node *node)
{
  struct_bgp_node *next;
  struct_bgp_node *start;
...
struct_bgp_node *
bgp_route_next_until (struct_bgp_node *node, struct_bgp_node *limit)
{
  struct_bgp_node *next;
  struct_bgp_node *start;
