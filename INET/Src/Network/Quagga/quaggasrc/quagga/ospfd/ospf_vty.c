...
int
ospf_oi_count (struct interface_FOO *ifp)
{
		      ...
		      struct ospf_vl_config_data *vl_config)
{
  struct crypt_key *ck;
  struct vty *vty;
  struct interface_FOO *ifp = vl_data->vl_oi->ifp;
...
ospf_vl_set_timers (struct ospf_vl_data *vl_data,
		    struct ospf_vl_config_data *vl_config)
{
  struct interface_FOO *ifp = ifp = vl_data->vl_oi->ifp;
       ...
       "The reference bandwidth in terms of Mbits per second\n")
{
  ...
  for (node = listhead (om->iflist__item); node; nextnode (node))
...
DEFUN (ip_ospf_cost,
       ...
       "Address of interface")
{
  struct interface_FOO *ifp = vty->index;
  u_int32_t cost;
  struct in_addr addr;
  int ret;
  struct ospf_if_params *params;
...
ospf_config_write (struct vty *vty)
{
  ...
  if (ospf != NULL)
	...
	vty_out (vty, " refresh timer %d%s",
      ...
      for (node = listhead (om->iflist__item); node; nextnode (node))
        {
          struct interface_FOO *ifp = getdata (node);
...
struct cmd_node interface_node_ospfd =
{
  INTERFACE_NODE,
  "%s(config-if)# ",
  1
...
struct cmd_node zebra_node_ospfd =
{
  ZEBRA_NODE,
  "%s(config-router)#",
};
...
struct cmd_node ospf_node_ospfd =
{
  OSPF_NODE,
  "%s(config-router)# ",
  1
