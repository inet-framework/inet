...
TCPDataTransferMode TCPSocket::strToTransfermode(const char* transferMode)
{
    if(0 == transferMode)
        throw cRuntimeError("Missing transferMode parameter at strToTransfermode() function");
...
void TCPSocket::readDataTransferModePar(cComponent &component)
{
    ...
    dataTransferMode = strToTransfermode(transferMode);
    if (dataTransferMode == TCP_TRANSFER_UNDEFINED)
