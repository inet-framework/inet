...
#ifndef __NEMOBINDINGCACHE_H__
#define __NEMOBINDINGCACHE_H__
...
class INET_API NemoBindingCache : public cSimpleModule
{
  private:
    ...
    struct NemoBindingCacheEntry
    {
        ...
        bool mobileRouter; // Mobile Router Flag. It means that HA supports NEMO
    ...
    typedef std::map<IPv6Address,NemoBindingCacheEntry> NemoBindingCache6; //The IPv6 Address KEY of this map is the HomeAddress of the MN
    NemoBindingCache6 nemoBindingCache;

    friend std::ostream& operator<<(std::ostream& os, const NemoBindingCacheEntry& nbce);

  public:
    NemoBindingCache();
    virtual ~NemoBindingCache();
  ...
  public:
    ...
    // fayruz 04.03.2015
    bool getMobileRouter(const IPv6Address& HoA) const;
};
