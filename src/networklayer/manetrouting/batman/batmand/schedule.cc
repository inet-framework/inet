...
//        forw_packet_tmp = NULL;
    ...
    for (OrigMap::iterator it = origMap.begin(); it != origMap.end(); ++it)
    {
        ...
        debug_output(4) << "count own bcast (schedule_own_packet): old = " << (unsigned)orig_node->bcast_own_sum[batman_if->if_num] << ", ";
        ...
        vectorAux.clear();
        debug_output(4) << "new = " << (unsigned)orig_node->bcast_own_sum[batman_if->if_num] << "\n";
    }
}


void Batman::schedule_forward_packet(OrigNode *orig_node, BatmanPacket *in, const ManetAddress &neigh, uint8_t directlink, int16_t hna_buff_len, BatmanIf *if_incoming, const simtime_t &curr_time)
{
...
//                    addr_to_string(bat_packet->orig, orig_str, ADDR_STR_LEN);
                ...
                debug_output(4) << (curr_packet_num > 0 ? "Forwarding" : (forw_node->own ? "Sending own" : "Forwarding")) << " " << (curr_packet_num > 0 ? "aggregated " : "") << "packet (originator " << bat_packet->getOrig() << ", seqno " << bat_packet->getSeqNumber() << ", TQ " << (unsigned)bat_packet->getTq() << ", TTL " << bat_packet->getTtl() << ", IDF " << (bat_packet->getFlags() & DIRECTLINK ? "on" : "off") << ") on interface " << batman_if->dev << "\n";
