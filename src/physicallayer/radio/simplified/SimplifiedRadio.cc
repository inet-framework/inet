...
void SimplifiedRadio::handleMessageWhenUp(cMessage *msg)
{
    ...
    {
        if (radioMode == OldIRadio::RADIO_MODE_RECEIVER)
        {
...
{
    if (transmissionState == TRANSMISSION_STATE_TRANSMITTING)
...
void SimplifiedRadio::handleCommand(int msgkind, cObject *ctrl)
{
    if (msgkind == RADIO_C_CONFIGURE)
    {
        ...
        if (newChannel != -1)
        {
            ...
            else if (transmissionState == TRANSMISSION_STATE_TRANSMITTING)
            {
...
void SimplifiedRadio::handleLowerMsgStart(SimplifiedRadioFrame* radioFrame)
{
    ...
    if (radioFrame->getArrivalTime() == simTime() && rcvdPower >= sensitivity && transmissionState != TRANSMISSION_STATE_TRANSMITTING && snrInfo.ptr == NULL)
    {
...
void SimplifiedRadio::setOldRadioChannel(int channel)
{
    Enter_Method_Silent();
...
void SimplifiedRadio::setRadioMode(RadioMode newRadioMode)
{
    ...
    if (radioMode != newRadioMode)
    {
        if (newRadioMode == RADIO_MODE_SLEEP || newRadioMode == RADIO_MODE_OFF)
            ...
            disconnectReceiver();
        else if (newRadioMode == OldIRadio::RADIO_MODE_TRANSCEIVER)
