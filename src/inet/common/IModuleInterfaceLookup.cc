...
//  - Ipv4 -> Tcp: Ipv4 module resolves the payload protocol from the IPv4 header before the lookup and searches using
//    a DispatchProtocolReq(tcp, SP_INDICATION) argument, Tcp module accepts because its ipIn gate declares
//    @interface[inet::queueing::IPassivePacketSink](protocol=tcp; service=indication)
