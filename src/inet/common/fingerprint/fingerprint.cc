...
void CustomFingerprintCalculator::addEvent(cEvent *event)
{
    if (addEvents) {
        ...
        if (eventMatcher == nullptr || eventMatcher->matches(&matchableEvent)) {
            ...
            if (module &&
                    (typeid(*module) == typeid(MultiNetworkLayerUpperMultiplexer)
                    || typeid(*module) == typeid(MultiNetworkLayerLowerMultiplexer)
                    || typeid(*module) == typeid(NetworkDatagramMultiplexer)
                    ))
                        ...
                        switch (ingredient) {
                            ...
                            case MODULE_FULL_PATH:
                                if (module != nullptr) {
                                    ...
                                    // MultiDYMO --> DYMO:
                                    while ((pos = fullPath.find(".dymo.ipv4")) != std::string::npos) {
                                        fullPath.erase(pos+5, 5);
                                    }
                                    while ((pos = fullPath.find(".dymo.ipv6")) != std::string::npos) {
                                        fullPath.erase(pos+5, 5);
                                    }
                                    while ((pos = fullPath.find(".dymo.generic")) != std::string::npos) {
                                        fullPath.erase(pos+5, 8);
                                    }
