...
void GroupCountRecorder::receiveSignal(cResultFilter *prev, simtime_t_cref t, OMNETPP5_CODE(long) OMNETPP6_CODE(intval_t) l, cObject *details) {
    std::stringstream s;
    s << l;
    collect(s.str());
}

void GroupCountRecorder::receiveSignal(cResultFilter *prev, simtime_t_cref t, OMNETPP5_CODE(unsigned long) OMNETPP6_CODE(uintval_t) l, cObject *details) {
    std::stringstream s;
    s << l;
    collect(s.str());
}
