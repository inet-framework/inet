...
// CRC-32/MPEG-2 lookup table. It uses the same polynomial as the table above, but since
// CRC-32/MPEG-2 is not reflected, the table is MSB-first (and so is the loop that uses it).
const uint32_t crc32_mpeg2_tab[] = {
...
uint32_t crc32_mpeg2(const uint8_t *buf, size_t bufsize, uint32_t crc)
{
    // Note: no XOR-ing with ~0U here, because the final XOR value is zero, so the
    // running CRC value and the returned one are the same; that is also why the
    // initial value 0xffffffff has to be supplied by the caller (as the default).
    const uint8_t *p = buf;
    while (bufsize--)
        crc = crc32_mpeg2_tab[((crc >> 24) ^ *p++) & 0xFF] ^ (crc << 8);
    return crc;
}
