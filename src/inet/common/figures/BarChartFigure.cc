...
int BarChartFigure::getItemIndex(const char *label, bool createIfMissing)
{
    for (size_t i = 0; i < bars.size(); i++)
        if (bars[i].label == label)
            return i;
    return createIfMissing ? addBar(label) : -1;
}

int BarChartFigure::addBar(const char *label)
{
    Bar newBar;
    newBar.label = label;
    newBar.barFigure = new cRectangleFigure("bar");
    newBar.barFigure->setFilled(true);
    newBar.barFigure->setLineColor(barLineColor);
    newBar.barFigure->setVisible(false);
    addFigure(newBar.barFigure);
    newBar.valueFigure = new cLabelFigure("value");
    newBar.valueFigure->setAnchor(ANCHOR_S);
    newBar.valueFigure->setFont(valueFont);
    newBar.valueFigure->setColor(valueColor);
    newBar.valueFigure->setVisible(false);
    addFigure(newBar.valueFigure);
    newBar.labelFigure = new cLabelFigure("label");
    newBar.labelFigure->setText(label);
    newBar.labelFigure->setAnchor(ANCHOR_NE);
    newBar.labelFigure->setAngle(labelAngle);
    newBar.labelFigure->setFont(labelFont);
    newBar.labelFigure->setColor(labelColor);
    addFigure(newBar.labelFigure);
    newBar.labelExtent = getTextExtent(labelFont, label);
    bars.push_back(newBar);
    layout();
    return bars.size() - 1;
}

const char *BarChartFigure::getBarLabel(int barIndex) const
{
    return bars.at(barIndex).label.c_str();
}

double BarChartFigure::getBarValue(int barIndex) const
{
    return bars.at(barIndex).value;
}

void BarChartFigure::clearBars()
{
    for (auto& bar : bars) {
...
void BarChartFigure::setValue(int barIndex, simtime_t timestamp, double value)
{
    if (barIndex < 0 || barIndex >= (int)bars.size())
        throw cRuntimeError(this, "Bar index %d is out of bounds, the figure has %d bars", barIndex, (int)bars.size());
...
{
    // the bars are displayed in sorted label order, so that the chart does not
    // reshuffle as bars appear
    std::vector<int> order(bars.size());
    std::iota(order.begin(), order.end(), 0);
    std::sort(order.begin(), order.end(), [&](int a, int b) { return bars[a].label < bars[b].label; });
