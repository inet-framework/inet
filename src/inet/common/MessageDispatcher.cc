...
void MessageDispatcher::arrived(cMessage *message, cGate *inGate,
                                const SendOptions &options, simtime_t time) {
...
bool MessageDispatcher::canPushSomePacket(cGate *inGate) const {
  ...
  for (int i = 0; i < size; i++) {
    ...
    if (consumer != nullptr && !dynamic_cast<MessageDispatcher *>(consumer) &&
        !consumer->canPushSomePacket(outGate->getPathEndGate()))
      return false;
  }
  return true;
}
...
bool MessageDispatcher::canPushPacket(Packet *packet, cGate *inGate) const {
  ...
  auto consumer = findConnectedModule<queueing::IPassivePacketSink>(outGate);
  return consumer != nullptr && !dynamic_cast<MessageDispatcher *>(consumer) &&
...
void MessageDispatcher::handleCanPushPacketChanged(cGate *outGate) {
  ...
  for (int i = 0; i < size; i++) {
    ...
    if (producer != nullptr && !dynamic_cast<MessageDispatcher *>(producer) &&
        outGate->getOwnerModule() != inGate->getOwnerModule())
...
cGate *MessageDispatcher::handlePacket(Packet *packet, cGate *inGate) {
  ...
  if (socketInd != nullptr) {
    ...
    if (it != socketIdToGateIndex.end()) {
      ...
      EV_INFO << "Dispatching packet to socket" << EV_FIELD(socketId)
              << EV_FIELD(inGate) << EV_FIELD(outGate) << EV_FIELD(packet)
              << EV_ENDL;
      return outGate;
    } else
      throw cRuntimeError("handlePacket(): Unknown socket: socketId = %d, "
  ...
  if (dispatchProtocolReq != nullptr) {
    ...
    // KLUDGE eliminate this by adding ServicePrimitive to every
    // DispatchProtocolReq
    if (servicePrimitive == static_cast<ServicePrimitive>(-1)) {
      if (packetProtocolTag != nullptr && dispatchProtocolReq->getProtocol() ==
    ...
    if (servicePrimitive == SP_REQUEST) {
      auto it =
          serviceToGateIndex.find(Key(protocol->getId(), servicePrimitive));
      if (it != serviceToGateIndex.end()) {
        auto outGate = gate("out", it->second);
        EV_INFO << "Dispatching packet to service"
                << EV_FIELD(protocol, *protocol) << EV_FIELD(servicePrimitive)
                << EV_FIELD(inGate) << EV_FIELD(outGate) << EV_FIELD(packet)
                << EV_ENDL;
        return outGate;
      } else {
        ...
        if (it != serviceToGateIndex.end()) {
          auto outGate = gate("out", it->second);
          EV_INFO << "Dispatching packet to any service"
                  << EV_FIELD(servicePrimitive) << EV_FIELD(inGate)
      ...
      auto it =
          protocolToGateIndex.find(Key(protocol->getId(), servicePrimitive));
      if (it != protocolToGateIndex.end()) {
        auto outGate = gate("out", it->second);
        EV_INFO << "Dispatching packet to protocol"
                << EV_FIELD(protocol, *protocol) << EV_FIELD(servicePrimitive)
                << EV_FIELD(inGate) << EV_FIELD(outGate) << EV_FIELD(packet)
                << EV_ENDL;
        return outGate;
        ...
        if (it != protocolToGateIndex.end()) {
          auto outGate = gate("out", it->second);
          EV_INFO << "Dispatching packet to any protocol"
                  << EV_FIELD(servicePrimitive) << EV_FIELD(inGate)
                  << EV_FIELD(outGate) << EV_FIELD(packet) << EV_ENDL;
          return outGate;
        } else
      ...
      throw cRuntimeError(
          "handlePacket(): Unknown service primitive: servicePrimitive = %d, "
          "pathStartGate = %s, pathEndGate = %s",
  ...
  throw cRuntimeError("handlePacket(): Unknown packet: %s(%s), pathStartGate = "
                      "%s, pathEndGate = %s",
                      packet->getName(), packet->getClassName(),
                      inGate->getPathStartGate()->getFullPath().c_str(),
                      inGate->getPathEndGate()->getFullPath().c_str());
}
...
cGate *MessageDispatcher::handleMessage(Message *message, cGate *inGate) {
  ...
  if (socketReq != nullptr) {
    ...
    if (it == socketIdToGateIndex.end())
      ...
      throw cRuntimeError(
          "handleMessage(): Socket is already registered: socketId = %d, "
          "current gateIndex = %d, new gateIndex = %d, pathStartGate = %s, "
          "pathEndGate = %s",
          socketReqId, it->second, inGate->getIndex(),
          inGate->getPathStartGate()->getFullPath().c_str(),
          inGate->getPathEndGate()->getFullPath().c_str());
  }
  ...
  if (socketInd != nullptr) {
    ...
    if (it != socketIdToGateIndex.end()) {
      auto outGate = gate("out", it->second);
      EV_INFO << "Dispatching message to socket" << EV_FIELD(socketId)
              << EV_FIELD(inGate) << EV_FIELD(outGate) << EV_FIELD(message)
              << EV_ENDL;
      return outGate;
    } else
      throw cRuntimeError("handleMessage(): Unknown socket: socketId = %d, "
                          "pathStartGate = %s, pathEndGate = %s",
                          socketId,
                          inGate->getPathStartGate()->getFullPath().c_str(),
                          inGate->getPathEndGate()->getFullPath().c_str());
  ...
  if (dispatchProtocolReq != nullptr) {
    ...
    if (servicePrimitive == SP_REQUEST) {
      ...
      if (it != serviceToGateIndex.end()) {
        auto outGate = gate("out", it->second);
        EV_INFO << "Dispatching message to service"
                << EV_FIELD(protocol, *protocol) << EV_FIELD(servicePrimitive)
                << EV_FIELD(inGate) << EV_FIELD(outGate) << EV_FIELD(message)
                << EV_ENDL;
        return outGate;
        ...
        if (it != serviceToGateIndex.end()) {
          auto outGate = gate("out", it->second);
          EV_INFO << "Dispatching message to any service"
                  << EV_FIELD(servicePrimitive) << EV_FIELD(inGate)
      ...
      if (it != protocolToGateIndex.end()) {
        auto outGate = gate("out", it->second);
        EV_INFO << "Dispatching message to protocol"
                << EV_FIELD(protocol, *protocol) << EV_FIELD(servicePrimitive)
                << EV_FIELD(inGate) << EV_FIELD(outGate) << EV_FIELD(message)
                << EV_ENDL;
        return outGate;
        ...
        if (it != protocolToGateIndex.end()) {
          auto outGate = gate("out", it->second);
          EV_INFO << "Dispatching message to any protocol"
                  << EV_FIELD(servicePrimitive) << EV_FIELD(inGate)
                  << EV_FIELD(outGate) << EV_FIELD(message) << EV_ENDL;
          return outGate;
        } else
      ...
      throw cRuntimeError(
          "handleMessage(): Unknown service primitive: servicePrimitive = %d, "
          "pathStartGate = %s, pathEndGate = %s",
          static_cast<int>(servicePrimitive),
          inGate->getPathStartGate()->getFullPath().c_str(),
          inGate->getPathEndGate()->getFullPath().c_str());
  ...
  throw cRuntimeError("handleMessage(): Unknown message: %s(%s), pathStartGate "
                      "= %s, pathEndGate = %s",
                      message->getName(), message->getClassName(),
                      inGate->getPathStartGate()->getFullPath().c_str(),
                      inGate->getPathEndGate()->getFullPath().c_str());
}

void MessageDispatcher::handleRegisterService(
    const Protocol &protocol, cGate *g, ServicePrimitive servicePrimitive) {
  Enter_Method("handleRegisterService");
  if (registeringProtocol != nullptr) {
    if (registeringProtocol != &protocol)
      throw cRuntimeError("handleRegisterService(): cannot register protocol "
                          "while another registration is being done");
  } else {
    registeringProtocol = &protocol;
    EV_INFO << "Handling service registration from " << printRegistrationPath()
            << EV_FIELD(protocol) << EV_FIELD(servicePrimitive)
            << EV_FIELD(gate, g) << EV_ENDL;
    auto key = Key(protocol.getId(), servicePrimitive);
    auto it = serviceToGateIndex.find(key);
    if (it != serviceToGateIndex.end()) {
      if (it->second != g->getIndex())
        throw cRuntimeError(
            "handleRegisterService(): service is already registered: "
            "protocolId = %d, protocolName = %s, servicePrimitive = %d, "
            "pathStartGate = %s, pathEndGate = %s",
            protocol.getId(), protocol.str().c_str(),
            static_cast<int>(servicePrimitive),
...
void MessageDispatcher::handleRegisterAnyService(
    cGate *g, ServicePrimitive servicePrimitive) {
  Enter_Method("handleRegisterAnyService");
  if (!registeringAny) {
    registeringAny = true;
    EV_INFO << "Handling any service registration from "
            << printRegistrationPath() << EV_FIELD(servicePrimitive)
            << EV_FIELD(gate, g) << EV_ENDL;
    ...
    if (it != serviceToGateIndex.end()) {
      if (it->second != g->getIndex())
        throw cRuntimeError(
            "handleRegisterAnyService(): any service is already registered: "
            "servicePrimitive = %d, pathStartGate = %s, pathEndGate = %s",
            static_cast<int>(servicePrimitive),
            g->getPathStartGate()->getFullPath().c_str(),
            g->getPathEndGate()->getFullPath().c_str());
    } else {
...
void MessageDispatcher::handleRegisterProtocol(
    const Protocol &protocol, cGate *g, ServicePrimitive servicePrimitive) {
  Enter_Method("handleRegisterProtocol");
  if (registeringProtocol != nullptr) {
    if (registeringProtocol != &protocol)
      throw cRuntimeError("handleRegisterProtocol(): cannot register protocol "
                          "while another registration is being done");
  } else {
    registeringProtocol = &protocol;
    EV_INFO << "Handling protocol registration from " << printRegistrationPath()
            << EV_FIELD(protocol) << EV_FIELD(servicePrimitive)
            << EV_FIELD(gate, g) << EV_ENDL;
    auto key = Key(protocol.getId(), servicePrimitive);
    ...
    if (it != protocolToGateIndex.end()) {
      if (it->second != g->getIndex())
        throw cRuntimeError(
            "handleRegisterProtocol(): protocol is already registered: "
            "protocolId = %d, protocolName = %s, servicePrimitive = %d, "
            "pathStartGate = %s, pathEndGate = %s",
            protocol.getId(), protocol.str().c_str(),
            static_cast<int>(servicePrimitive),
...
void MessageDispatcher::handleRegisterAnyProtocol(
    cGate *g, ServicePrimitive servicePrimitive) {
  Enter_Method("handleRegisterAnyProtocol");
  if (!registeringAny) {
    registeringAny = true;
    EV_INFO << "Handling any protocol registration from "
            << printRegistrationPath() << EV_FIELD(servicePrimitive)
            << EV_FIELD(gate, g) << EV_ENDL;
    auto key = Key(-1, servicePrimitive);
    auto it = protocolToGateIndex.find(key);
    if (it != protocolToGateIndex.end()) {
      if (it->second != g->getIndex())
        throw cRuntimeError(
            "handleRegisterAnyProtocol(): any protocol is already registered: "
            "servicePrimitive = %d, pathStartGate = %s, pathEndGate = %s",
            static_cast<int>(servicePrimitive),
            g->getPathStartGate()->getFullPath().c_str(),
            g->getPathEndGate()->getFullPath().c_str());
...
void MessageDispatcher::handleRegisterInterface(
    const NetworkInterface &interface, cGate *out, cGate *in) {
  Enter_Method("handleRegisterInterface");
  EV_INFO << "Handling interface registration" << EV_FIELD(interface)
          << EV_FIELD(out, out) << EV_FIELD(in) << EV_ENDL;
  auto it = interfaceIdToGateIndex.find(interface.getInterfaceId());
  if (it != interfaceIdToGateIndex.end()) {
    if (it->second != out->getIndex())
      throw cRuntimeError("handleRegisterInterface(): interface is already "
                          "registered: interfaceId = %d, interfaceName = %s, "
                          "pathStartGate = %s, pathEndGate = %s",
                          interface.getInterfaceId(), interface.str().c_str(),
                          out->getPathStartGate()->getFullPath().c_str(),
                          out->getPathEndGate()->getFullPath().c_str());
  } else {
