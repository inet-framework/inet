    ...
    for (int i = 0; i < size; i++) {
        if (i != gate->getIndex()) {
            ...
            if (referencedGate != nullptr) {
                ...
                if (result != nullptr) {
                    // precedence rule: a module's direct claim shadows a message dispatcher's
                    // transitive claim, because dispatchers answer as in-path routers on behalf
                    // of the modules reachable through them; in cyclic topologies (e.g. the
                    // parallel protocol sublayer arms between two dispatchers) the transitive
                    // answer merely duplicates the direct claim; two direct claims or two
                    // dispatcher claims are genuinely ambiguous and remain hard errors
                    bool resultIsMessageDispatcher = dynamic_cast<MessageDispatcher *>(result->getOwnerModule()) != nullptr;
                    bool referencedModuleIsMessageDispatcher = dynamic_cast<MessageDispatcher *>(referencedModule) != nullptr;
                    if (!resultIsMessageDispatcher && referencedModuleIsMessageDispatcher)
                        continue; // keep the direct claim, keep scanning for conflicting direct claims
                    else if (resultIsMessageDispatcher && !referencedModuleIsMessageDispatcher) {
                        result = referencedGate; // the direct claim replaces the dispatcher's transitive claim
                        continue;
                    }
                    else
