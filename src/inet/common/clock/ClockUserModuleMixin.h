...
class INET_API ClockUserModuleMixin : public T, public cListener
{
  ...
  public:
    ...
    virtual void rescheduleClockEventAt(clocktime_t time, ClockEvent* msg) { T::rescheduleAt(time, T::cancelEvent(msg)); }
    virtual void rescheduleClockEventAfter(clocktime_t time, ClockEvent* msg) { T::rescheduleAfter(time, T::cancelEvent(msg)); }
