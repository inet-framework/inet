...
class INET_API EthernetSocket : public SocketBase, public IEthernet::ICallback
{
  ...
  public:
    virtual void setOutputGate(cGate *gate) override {
        ...
        ethernet.reference(gate, false);
        if (ethernet.getNullable() == nullptr)
            throw cRuntimeError("Cannot find the IEthernet module interface for this socket "
                                 "(gate = %s); raw Ethernet sockets require socket support in "
                                 "the ethernet layer of this node, set hasSocketSupport = true "
                                 "on it (e.g. **.ethernet.hasSocketSupport = true)",
