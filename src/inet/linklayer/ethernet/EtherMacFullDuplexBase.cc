...
const double EtherMacFullDuplexBase::SPEED_OF_LIGHT_IN_CABLE = 200000000.0;

const EtherMacFullDuplexBase::EtherDescr EtherMacFullDuplexBase::nullEtherDescr = {
    0.0,
    0.0,
    0,
    B(0),
...
const EtherMacFullDuplexBase::EtherDescr EtherMacFullDuplexBase::etherDescrs[NUM_OF_ETHERDESCRS] = {
    {
        ETHERNET_TXRATE,
        0.5 / ETHERNET_TXRATE,
        0,
...
simsignal_t EtherMacFullDuplexBase::rxPkOkSignal = registerSignal("rxPkOk");
simsignal_t EtherMacFullDuplexBase::txPausePkUnitsSignal = registerSignal("txPausePkUnits");
simsignal_t EtherMacFullDuplexBase::rxPausePkUnitsSignal = registerSignal("rxPausePkUnits");

simsignal_t EtherMacFullDuplexBase::transmissionStateChangedSignal = registerSignal("transmissionStateChanged");
simsignal_t EtherMacFullDuplexBase::receptionStateChangedSignal = registerSignal("receptionStateChanged");
...
void EtherMacFullDuplexBase::initialize(int stage)
{
...
void EtherMacFullDuplexBase::initializeQueue()
{
...
void EtherMacFullDuplexBase::initializeStatistics()
{
...
void EtherMacFullDuplexBase::handleStartOperation(LifecycleOperation *operation)
{
...
void EtherMacFullDuplexBase::handleCrashOperation(LifecycleOperation *operation)
{
//    clearQueue();
    connected = false;
    interfaceEntry->setCarrier(false);
...
void EtherMacFullDuplexBase::processAtHandleMessageFinished()
{
...
void EtherMacFullDuplexBase::receiveSignal(cComponent *source, simsignal_t signalID, cObject *obj, cObject *details)
{
    Enter_Method_Silent();
...
void EtherMacFullDuplexBase::encapsulate(Packet *frame)
{
...
void EtherMacFullDuplexBase::decapsulate(Packet *packet)
{
...
bool EtherMacFullDuplexBase::verifyCrcAndLength(Packet *packet)
{
    EV_STATICCONTEXT;
...
void EtherMacFullDuplexBase::refreshConnection()
{
    Enter_Method_Silent();
...
bool EtherMacFullDuplexBase::dropFrameNotForUs(Packet *packet, const Ptr<const EthernetMacHeader>& frame)
{
...
void EtherMacFullDuplexBase::printParameters()
{
    // Dump parameters
...
void EtherMacFullDuplexBase::finish()
{
    {
        simtime_t t = simTime();
...
void EtherMacFullDuplexBase::refreshDisplay() const
{
...
void EtherMacFullDuplexBase::changeTransmissionState(MacTransmitState newState)
{
    transmitState = newState;
    emit(transmissionStateChangedSignal, newState);
}

void EtherMacFullDuplexBase::changeReceptionState(MacReceiveState newState)
{
