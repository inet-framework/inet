...
MacUtils::MacUtils(IMacParameters *params, IRateSelection *rateSelection) :
...
simtime_t MacUtils::getAckEarlyTimeout() const {
    ...
    return params->getSifsTime() + params->getSlotTime()
...
Ieee80211RTSFrame *MacUtils::buildRtsFrame(
        Ieee80211DataOrMgmtFrame *dataFrame) const {
    return buildRtsFrame(dataFrame, getFrameMode(dataFrame));
}

Ieee80211RTSFrame *MacUtils::buildRtsFrame(Ieee80211DataOrMgmtFrame *dataFrame,
...
Ieee80211RTSFrame *MacUtils::buildRtsFrame(const MACAddress& receiverAddress,
        simtime_t duration) const {
    ...
    setFrameMode(rtsFrame,
            rateSelection->getModeForControlFrame(dataFrame, rtsFrame));
    return rtsFrame;
}
...
Ieee80211CTSFrame *MacUtils::buildCtsFrame(Ieee80211RTSFrame *rtsFrame) const {
    ...
    Ieee80211DataOrMgmtFrame *dataFrame = new Ieee80211DataOrMgmtFrame("DATA");
    setFrameMode(frame,
            rateSelection->getModeForControlFrame(dataFrame, frame));
    return frame;
}

Ieee80211ACKFrame *MacUtils::buildAckFrame(
        Ieee80211DataOrMgmtFrame *dataFrame) const {
...
Ieee80211Frame *MacUtils::setFrameMode(Ieee80211Frame *frame,
...
const IIeee80211Mode *MacUtils::getFrameMode(Ieee80211Frame *frame) const {
    Ieee80211TransmissionRequest *ctrl = check_and_cast<
            Ieee80211TransmissionRequest*>(frame->getControlInfo());
    return ctrl->getMode();
}

bool MacUtils::isForUs(Ieee80211Frame *frame) const {
    return frame->getReceiverAddress() == params->getAddress()
            || (frame->getReceiverAddress().isMulticast() && !isSentByUs(frame));
}
...
simtime_t MacUtils::getTxopLimit(AccessCategory ac,
    ...
    case AC_VI:
        if (dynamic_cast<const Ieee80211DsssMode*>(mode)
                || dynamic_cast<const Ieee80211HrDsssMode*>(mode))
            return ms(6.016).get();
        else if (dynamic_cast<const Ieee80211HTMode*>(mode)
                || dynamic_cast<const Ieee80211OFDMMode*>(mode))
            return ms(3.008).get();
        else
            return 0;
    case AC_VO:
...
int MacUtils::cmpMgmtOverData(Ieee80211DataOrMgmtFrame *a,
...
int MacUtils::cmpMgmtOverMulticastOverUnicast(Ieee80211DataOrMgmtFrame *a,
        Ieee80211DataOrMgmtFrame *b) {
    int aPri = dynamic_cast<Ieee80211ManagementFrame*>(a) ? 2 :
               a->getReceiverAddress().isMulticast() ? 1 : 0;
    int bPri = dynamic_cast<Ieee80211ManagementFrame*>(a) ? 2 :
