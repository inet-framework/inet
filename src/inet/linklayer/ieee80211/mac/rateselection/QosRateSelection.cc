...
void QosRateSelection::updateModes()
{
    ...
    auto newResponseBlockAckFrameMode = responseBlockAckFrameBitrate == -1 ? nullptr : modeSet->getMode(bps(responseBlockAckFrameBitrate), Hz(par("responseBlockAckFrameBandwidth")), par("responseBlockAckFrameNumSpatialStreams"), par("responseBlockAckFrameGuardInterval"), true);
...
{
    if (mode == nullptr || peerAddress.isMulticast() || !mib)
        return mode;
    if (mode->getVhtMcsIndex() >= 0)
        return selectPeerCompatibleVhtMode(modeSet, mib->getLocalVhtCapabilities(), mib->getLocalVhtOperation(),
                mib->findPeerVhtState(peerAddress), mode);
