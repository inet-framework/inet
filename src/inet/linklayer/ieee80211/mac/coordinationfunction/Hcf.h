...
class INET_API Hcf : public ICoordinationFunction, public IFrameSequenceHandler::ICallback, public IChannelAccess::ICallback, public ITx::ICallback, public IProcedureCallback, public IBlockAckAgreementHandlerCallback, public ModeSetListener
{
  ...
  protected:
    ...
    // removing its siblings so queue notifications cannot report the same logical
    ...
    // notifications of a bulk queue removal and is cleared at the next event.
  ...
  protected:
    ...
    virtual void handlePacketRemoved(Packet *packet, queueing::IPacketQueue::PacketRemovalReason reason);
