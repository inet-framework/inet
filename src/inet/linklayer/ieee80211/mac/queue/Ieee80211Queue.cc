...
{
    int aPri = std::dynamic_pointer_cast<Ieee80211ManagementHeader>(a->peekHeader<Ieee80211Frame>()) ? 1 : 0;  //TODO there should really exist a high-performance isMgmtFrame() function!
    int bPri = std::dynamic_pointer_cast<Ieee80211ManagementHeader>(b->peekHeader<Ieee80211Frame>()) ? 1 : 0;
    return bPri - aPri;
}
...
int PendingQueue::cmpMgmtOverMulticastOverUnicast(Packet *a, Packet *b)
{
    ...
    int aPri = std::dynamic_pointer_cast<Ieee80211ManagementHeader>(aHeader) ? 2 : aHeader->getReceiverAddress().isMulticast() ? 1 : 0;
    int bPri = std::dynamic_pointer_cast<Ieee80211ManagementHeader>(bHeader) ? 2 : bHeader->getReceiverAddress().isMulticast() ? 1 : 0;
    return bPri - aPri;
}
