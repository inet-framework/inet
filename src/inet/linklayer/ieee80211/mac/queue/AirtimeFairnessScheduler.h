 ...
 *
 * Stations come and go, so the input gates are not all known at network setup time: the
 * ~DynamicClassifier of the enclosing queue wires up a new branch as each receiver first
 * appears. The scheduler picks such an input up by listening for the model change
 * notification of its own input gate being connected, rather than by being called into.
 *
 ...
 */
class INET_API AirtimeFairnessScheduler : public queueing::PacketSchedulerBase, public virtual queueing::IPacketCollection, public cListener
{
  protected:
    // parameters
    ...
    int lastServedIndex = -1; // input gate index served by the previous schedulePacket() call
    simtime_t lastServedDeficit = SIMTIME_ZERO; // deficit of that station when it was served
  ...
  protected:
    ...
    /**
     * Takes an input gate connected at runtime (a per-station branch created on demand by
     * ~DynamicClassifier) into the rotation: appends the provider/producer references and
     * the matched gate.
     */
  ...
  public:
    /**
     * Notices this scheduler's own input gates being connected at runtime; see addInput().
     */
