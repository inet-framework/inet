...
FcsMetadata getIeee80211FcsMetadata(const Packet *packet, b frontOffset, b backOffset)
{
    ...
    // FcsInd describes the imported MAC frame, not arbitrary ranges or aggregate members.
    // Popped data and protocol-tag offsets also restrict the view of that original frame.
    auto protocolTag = packet->findTag<PacketProtocolTag>();
    if (frontOffset == b(0) && backOffset == b(0) && packet->getDataLength() == packet->getTotalLength() &&
            protocolTag != nullptr && protocolTag->getProtocol() == &Protocol::ieee80211Mac &&
            protocolTag->getFrontOffset() == b(0) && protocolTag->getBackOffset() == b(0)) {
        if (auto fcsInd = packet->findTag<FcsInd>()) {
            metadata.isPresent = fcsInd->getHasFcs();
        ...
        const physicallayer::IReception *reception)
{
    ...
    if (mode != nullptr) {
        // Radiotap Flags 0x02 describes the selected legacy preamble, independently of FCS.
        // https://www.radiotap.org/fields/Flags.html
        if (auto preambleMode = dynamic_cast<const physicallayer::Ieee80211HrDsssPreambleMode *>(mode->getPreambleMode()))
            fields.hasShortPreamble = preambleMode->getPreambleType() == physicallayer::IEEE80211_HRDSSS_PREAMBLE_TYPE_SHORT;
...
std::vector<uint8_t> serializeRadiotapHeader(const RadiotapPpduFields& fields, const RadiotapRecordMetadata& metadata)
{
    ...
    bytes.push_back((fields.hasShortPreamble ? RADIOTAP_F_SHORTPRE : 0) |
            (metadata.hasFcs ? RADIOTAP_F_FCS : 0) | (metadata.hasBadFcs ? RADIOTAP_F_BADFCS : 0));
    if (fields.hasRate) {
