...
void Gptp::synchronize()
{
    ...
    ppm rateDifference = unit(gmRateRatio.toDouble() - 1);
...
void Gptp::sendFollowUp(int portId)
{
    ...
    if (gptpNodeType == MASTER_NODE) {
        ...
        clocktime_t residenceTime = process.syncTransmissionStartUnsychronized - syncReceiverProcess.syncReceptionStartUnsychronized;
        ASSERT(residenceTime >= 0);
        ASSERT(pdelay >= 0);
        followUp->setCorrectionField(syncReceiverProcess.sync->getCorrectionField() + residenceTime + pdelay);
...
void Gptp::computeGmRateRatio()
{
    ...
    if (previousSourceTime != -1 && previousLocalTime != -1) {
        ...
        gmRateRatio = ClockTimeScale::fromDouble(sourceTimeDifference / localTimeDifference);
        ppm gmDriftRate = unit(gmRateRatio.toDouble() - 1);
...
void Gptp::computeNeighborRateRatio()
{
    ...
    if (previousCorrectedResponderEventTimestamp != -1 && previousPdelayRespEventIngressTimestamp != -1) {
        ...
        neighborRateRatio = ClockTimeScale::fromDouble(correctedResponderEventTimestampDifference / pdelayRespEventIngressTimestampDifference);
        ppm neighborDriftRate = unit(neighborRateRatio.toDouble() - 1);
