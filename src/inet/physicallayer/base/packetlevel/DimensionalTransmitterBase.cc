...
void DimensionalTransmitterBase::parseFrequencyGains(const char *text)
{
    if (strcmp(text, "smaller s 0dB either e 0dB greater")) {
        ...
        delete firstFrequencyInterpolator;
...
math::Function<W, simtime_t, Hz> *DimensionalTransmitterBase::createPowerFunction(const simtime_t startTime, const simtime_t endTime, Hz carrierFrequency, Hz bandwidth, W power) const
{
    if (timeGains.size() == 0 && frequencyGains.size() == 0)
        return new math::TwoDimensionalBoxcarFunction<W, simtime_t, Hz>(startTime, endTime, carrierFrequency - bandwidth / 2, carrierFrequency + bandwidth / 2, power);
    else {
        math::Function<double, simtime_t> *timeGainFunction;
        if (timeGains.size() != 0) {
            auto centerTime = (startTime + endTime) / 2;
            auto duration = endTime - startTime;
            std::map<simtime_t, std::pair<double, const math::Interpolator<simtime_t, double> *>> ts;
            ...
            timeGainFunction = new math::OneDimensionalInterpolatedFunction<double, simtime_t>(ts);
        }
        else
            timeGainFunction = new math::OneDimensionalBoxcarFunction<double, simtime_t>(startTime, endTime, 1);
        math::Function<double, Hz> *frequencyGainFunction;
        if (frequencyGains.size() != 0) {
            auto startFrequency = carrierFrequency - bandwidth / 2;
            auto endFrequency = carrierFrequency + bandwidth / 2;
            std::map<Hz, std::pair<double, const math::Interpolator<Hz, double>*>> fs;
            ...
            frequencyGainFunction = new math::OneDimensionalInterpolatedFunction<double, Hz>(fs);
        }
        else
            frequencyGainFunction = new math::OneDimensionalBoxcarFunction<double, Hz>(carrierFrequency - bandwidth / 2, carrierFrequency + bandwidth / 2, 1);
        auto gainFunction = new math::OrthogonalCombinatorFunction<simtime_t, Hz>(timeGainFunction, frequencyGainFunction);
        auto powerFunction = new math::ConstantFunction<W, simtime_t, Hz>(power);
        return math::Function<W, simtime_t, Hz>::multiply(powerFunction, gainFunction);
    }
}
