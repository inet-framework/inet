...
const ITransmission *Ieee802154UWBIRTransmitter::createTransmission(const IRadio *transmitter, const Packet *packet, const simtime_t startTime) const
{
    ...
    return new DimensionalTransmission(transmitter, packet, startTime, endTime, -1, -1, -1, startPosition, endPosition, startOrientation, endOrientation, bit(-1), packet->getPacketLength(), cfg.bitrate, nullptr, cfg.centerFrequency, cfg.bandwidth, powerMapping);
}
