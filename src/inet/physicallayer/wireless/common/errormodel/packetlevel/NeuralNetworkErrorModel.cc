...
std::ostream& NeuralNetworkErrorModel::printToStream(std::ostream& stream, int level, int evFlasgs) const
{
...
double NeuralNetworkErrorModel::computePacketErrorRate(const ISnir *snir, IRadioSignal::SignalPart part) const
{
    ...
    int timeDivision = (transmission->getHeaderDuration() + transmission->getDataDuration()) / transmission->getMode()->getDataMode()->getSymbolInterval();
...
std::vector<float> NeuralNetworkErrorModel::fillSnirTensor(const DimensionalSnir *snir, int timeDivision, int frequencyDivision) const
{
    ...
    auto endTime = reception->getDataEndTime();
    auto transmission = check_and_cast<const Ieee80211Transmission *>(reception->getTransmission());
    auto centerFrequency = check_and_cast<const INarrowbandSignalAnalogModel *>(reception->getAnalogModel())->getCenterFrequency();
...
std::string NeuralNetworkErrorModel::computeModelName(const ISnir *snir) const
{
    auto reception = snir->getReception();
    auto transmission = check_and_cast<const Ieee80211Transmission *>(reception->getTransmission());
    auto radio = check_and_cast<const Radio *>(transmission->getTransmitterRadio());
    auto transmissionAnalogModel = check_and_cast<const INarrowbandSignalAnalogModel *>(transmission->getAnalogModel());
    auto centerFrequency = transmissionAnalogModel->getCenterFrequency();
    auto bandwidth = transmissionAnalogModel->getBandwidth();
    auto bitrate = transmission->getMode()->getDataMode()->getGrossBitrate();
    auto modulation = transmission->getMode()->getDataMode()->getModulation();
