...
#include "inet/physicallayer/wireless/common/errormodel/packetlevel/Ieee80211RadioErrorModelEvaluator.h"
...
DimensionalNoise *createNoise(std::vector<double> snirs, const IReception *reception, int frequencyDivision, int timeDivision, SimTime startTime, SimTime endTime) {
        // create noise
    auto narrowbandSignal = check_and_cast<const INarrowbandSignalAnalogModel *>(reception->getAnalogModel());
    auto centerFrequency = narrowbandSignal->getCenterFrequency();
...
const Ieee80211Transmission *createLayeredTransmission(const IRadio *radio, const ITransmitter *transmitter, Packet *transmittedPacket) {

    // create transmission
    simtime_t startTime = 0;
    auto transmission = check_and_cast<const Ieee80211Transmission *>(transmitter->createTransmission(radio, transmittedPacket, startTime));
...
const DimensionalSnir *createLayeredSnir(const Reception *reception, const DimensionalNoise *noise) {
    // create snir
...
const ITransmission *createNonLayeredTransmission(const IRadio *radio, const ITransmitter *transmitter, Packet *transmittedPacket) {
    ...
    //if (auto forwardErrorCorrection = transmission->getBitModel()->getForwardErrorCorrection())
    //    bitrate *= forwardErrorCorrection->getCodeRate();
    auto modulation = transmission->getMode()->getDataMode()->getModulation();
    auto ofdmModulation = dynamic_cast<const Ieee80211OfdmModulation *>(modulation);
    auto subcarrierModulation = ofdmModulation != nullptr ? ofdmModulation->getSubcarrierModulation() : nullptr;
    //auto transmittedSymbols = transmission-> getSymbolModel()->getAllSymbols();
    // TODO: time division doesn't take into account that the preamble doesn't contain symbols
    int timeDivision = transmission->getDuration() / transmission->getMode()->getDataMode()->getSymbolInterval();
    int frequencyDivision = ofdmModulation != nullptr ? ofdmModulation->getNumSubcarriers() : 1;
    if (frequencyDivision != 52)
        throw cRuntimeError("wrong number of OFDM subcarriers!");
...
const DimensionalSnir *createNonLayeredSnir(const Reception *reception, const DimensionalNoise *noise) {
    // create snir
...
void Ieee80211RadioErrorModelEvaluator::evaluateErrorModel()
{
    ...
    while (true) {
        ...
        if (auto analogModel = dynamic_cast<const DimensionalMediumAnalogModel *>(radioMedium->getAnalogModel()))
        ...
        const Reception *reception = check_and_cast<const Reception*>(radioMedium->getAnalogModel()->computeReception(radio, transmission, arrival));
        ...
        */
        ...
        if (auto analogModel = dynamic_cast<const DimensionalMediumAnalogModel *>(radioMedium->getAnalogModel())) {
            auto lsnir = createLayeredSnir(reception, noise);
        ...
        if (auto analogModel = dynamic_cast<const DimensionalMediumAnalogModel *>(radioMedium->getAnalogModel())) {
            auto dsnir = createNonLayeredSnir(reception, noise);
        ...
        for (int i = 0; i < repeatCount; i++) {
            ...
            if (auto errorRateInd = receivedPacket->findTag<ErrorRateInd>()) {
