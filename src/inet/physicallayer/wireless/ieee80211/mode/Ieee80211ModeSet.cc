    ...
    htOperationSupported(htOperationSupported)
{
    ...
    // modeled OperationalRateSet is mandatory-first and bounded to eight
    // entries (IEEE Std 802.11-2024, 9.4.2.3 and 11.1.4.6).
    for (bool mandatory : {true, false}) {
        for (const auto& entry : this->entries) {
            if (entry.isLegacyOperational && entry.isMandatory == mandatory && this->legacyOperationalModes.size() < 8)
