...
int Ieee80211BandBase::getStandardChannelNumber(int channelIndex) const
{
    throw cRuntimeError("Band '%s' has no standards channel-number mapping for internal channel index %d", getName(), channelIndex);
}

int Ieee80211BandBase::getChannelIndex(int standardChannelNumber) const
{
...
bool Ieee80211BandBase::isHt40OperationSupported(int primaryChannelIndex, int secondaryChannelOffset) const
{
...
int Ieee80211EnumeratedBand::getStandardChannelNumber(int channelIndex) const
{
    ...
    if (standardChannelNumbers.empty())
        return Ieee80211BandBase::getStandardChannelNumber(channelIndex);
...
{
    if (standardChannelNumbers.empty())
        return Ieee80211BandBase::getChannelIndex(standardChannelNumber);
