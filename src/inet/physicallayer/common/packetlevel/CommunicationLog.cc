...
void CommunicationLog::writeTransmission(const IRadio *transmitter, const ISignal *radioFrame)
{
    ...
    output << "T " << transmitterRadio->getFullPath() << " " << transmitterRadio->getId() << " "
           << "M " << check_and_cast<const Signal *>(radioFrame)->getName() << " " << transmission->getId() << " "
           << "S " << transmission->getStartTime() << " " << transmission->getStartPosition() << " -> "
           << "E " << transmission->getEndTime() << " " << transmission->getEndPosition() << endl;
}

void CommunicationLog::writeReception(const IRadio *receiver, const ISignal *radioFrame)
{
    const ITransmission *transmission = radioFrame->getTransmission();
