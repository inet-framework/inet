...
{
  protected:
    const math::Function<W, simtime_t, Hz> *power;

  public:
    DimensionalSignalAnalogModel(const simtime_t duration, Hz carrierFrequency, Hz bandwidth, const math::Function<W, simtime_t, Hz> *power);
    virtual ~DimensionalSignalAnalogModel() { delete power; }

    virtual std::ostream& printToStream(std::ostream& stream, int level) const override;

    virtual const math::Function<W, simtime_t, Hz> *getPower() const override { return power; }
...
{
  public:
    DimensionalTransmissionSignalAnalogModel(const simtime_t duration, Hz carrierFrequency, Hz bandwidth, const math::Function<W, simtime_t, Hz> *power);
};
...
{
  public:
    DimensionalReceptionSignalAnalogModel(const simtime_t duration, Hz carrierFrequency, Hz bandwidth, const math::Function<W, simtime_t, Hz> *power);
};
