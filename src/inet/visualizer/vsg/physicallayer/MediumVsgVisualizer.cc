...
void MediumVsgVisualizer::refreshRingTransmissionNode(const ITransmission *transmission, ::vsg::Node *node) const
{
    ...
    // uniform (MediumOsgVisualizer::refreshRingTransmissionNode): at a fast animation the wavefront
    // sweeps many wavelengths per frame, so the ripple would strobe — fading it toward a smooth disc
    // avoids that. signalWaveFadingAnimationSpeedFactor tunes the onset. Guard the divisors so a zero
    // animation speed (e.g. a non-animating environment) yields 1.0 — the same value OSG's min() gives.
    double animationSpeed = getSimulation()->getEnvir()->getAnimationSpeed();
