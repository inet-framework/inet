    ...
    if (message->isSelfMessage()) {
        if (message == timer) {
            ...
            if (packet->isUpdate()) {
                ...
                pushOrSendPacketProgress(packet, outputGate, consumerGate, consumer, progressTag->getDatarate(), progressTag->getPosition(), progressTag->getExtraProcessableLength(), packet->getTransmissionId());
            }
            else
...
void InterpacketGapInserter::receivePacketStart(cPacket *cpacket, cGate *gate, double datarate)
{
    ...
    pushOrSendPacketStart(packet, outputGate, consumerGate, consumer, bps(datarate), packet->getTransmissionId());
}
...
{
    auto packet = check_and_cast<Packet *>(cpacket);
    pushOrSendPacketProgress(packet, outputGate, consumerGate, consumer, bps(datarate), b(bitPosition), b(extraProcessableBitLength), packet->getTransmissionId());
}
...
void InterpacketGapInserter::pushOrSendOrSchedulePacketProgress(Packet *packet, cGate *gate, bps datarate, b position, b extraProcessableLength)
{
    ...
    if (progress == nullptr || !progress->isScheduled()) {
        if (packet->getTotalLength() == position + extraProcessableLength)
            handlePacketProcessed(packet);
        pushOrSendPacketProgress(packet, outputGate, consumerGate, consumer, datarate, position, extraProcessableLength, packet->getTransmissionId());
    }
    else {
        EV_INFO << "Inserting packet gap before" << EV_FIELD(packet) << EV_ENDL;
