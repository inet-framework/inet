...
void StreamThroughReceiver::receivePacketStart(cPacket *cpacket, cGate *gate, bps datarate)
{
    ...
    pushOrSendPacketStart(packet, outputGate, consumer.getReferencedGate(), consumer, datarate, rxSignal->getTransmissionId());
}
...
void StreamThroughReceiver::receivePacketEnd(cPacket *cpacket, cGate *gate, bps datarate)
{
    ...
    if (rxSignal) {
        ...
        pushOrSendPacketEnd(packet, outputGate, consumer.getReferencedGate(), consumer, rxSignal->getTransmissionId());
        delete rxSignal;
        rxSignal = nullptr;
    }
    else {
