...
{
    struct fmemopen_read_state *read_cookie = (struct fmemopen_read_state *) cookie;
    int bytes = 0;
...
FILE *fmemopen(char *str, size_t size, const char *mode)
{
    ...
    read_cookie = (struct fmemopen_read_state*) calloc(1, sizeof(struct fmemopen_read_state));
