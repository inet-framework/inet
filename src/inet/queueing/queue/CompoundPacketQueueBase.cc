...
void CompoundPacketQueueBase::receiveSignal(cComponent *source, simsignal_t signal, cObject *object, cObject *details)
{
    ...
    if (signal == IPacketQueue::packetQueueDepartureSignal) {
        ...
        numDroppedPackets++;
        // Child queues already publish departures, and shared buffers notify
        // the owning queue before dropping. Only translate drops outside those
        // boundaries, such as input filters rejecting a packet before enqueue.
        auto module = dynamic_cast<cModule *>(source);
        while (module != nullptr && module != this) {
            if (dynamic_cast<IPacketQueue *>(module) != nullptr || dynamic_cast<IPacketBuffer *>(module) != nullptr)
                break;
            module = module->getParentModule();
        }
        if (source != this && module == this)
            notifyPacketRemoved(check_and_cast<Packet *>(object), IPacketQueue::PacketRemovalReason::DROPPED);
    }
