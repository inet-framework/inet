...
class Batman : public ManetRoutingBase
{
  ...
  private:
    ...
    void update_orig(OrigNode *orig_node,const BatmanPacket* in, const L3Address &neigh, BatmanIf *if_incoming, const HnaElement *hna_recv_buff, int16_t hna_buff_len, uint8_t is_duplicate, const simtime_t &curr_time );
    void purge_orig(const simtime_t &curr_time);
    void choose_gw(void);
    void update_routes(OrigNode *orig_node, NeighNode *, const HnaElement * hna_recv_buff, int16_t hna_buff_len);
    ...
    int isBidirectionalNeigh(OrigNode *orig_node, OrigNode *orig_neigh_node, const Ptr<BatmanPacket>&  in, const simtime_t &recv_time, BatmanIf *if_incoming);
    uint8_t count_real_packets(const BatmanPacket* in, const L3Address &neigh, BatmanIf *if_incoming);
    void schedule_own_packet(BatmanIf *batman_if);
    void schedule_forward_packet(OrigNode *orig_node, const Ptr<BatmanPacket>& in, const L3Address &neigh, uint8_t directlink, int16_t hna_buff_len, BatmanIf *if_incoming, const simtime_t &curr_time);
    void appendPacket(Packet *oldPacket,const Ptr<BatmanPacket>& packetToAppend);
    void send_outstanding_packets(const simtime_t &);
    int8_t send_udp_packet(Packet *, int32_t, const L3Address &, int32_t send_sock, BatmanIf *batman_if);
    ...
    void _hna_global_add(OrigNode *orig_node, const HnaElement *hna_element);
    ...
    void hna_global_add(OrigNode *orig_node, const HnaElement *new_hna, int16_t new_hna_len);
    void hna_global_update(OrigNode *orig_node, const HnaElement *new_hna, int16_t new_hna_len, NeighNode *);
    ...
    //build packets
    virtual Packet *buildDefaultBatmanPkt(const BatmanIf *);
