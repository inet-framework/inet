...
void Batman::schedule_own_packet(BatmanIf *batman_if)
{
    ...
    if ((hna_list.size() > 0) && (batman_if->if_num == 0)) {
        auto pkt = forw_node_new->pack_buff->removeAtFront<BatmanPacket>();
        pkt->setHnaMsgArraySize(hna_buff_local.size());
        pkt->setChunkLength(pkt->getChunkLength() + B((hna_buff_local.size() * BATMAN_HNA_MSG_SIZE)));
        ...
        forw_node_new->pack_buff->insertAtFront(pkt);
    }
...
void Batman::schedule_forward_packet(OrigNode *orig_node, const Ptr<BatmanPacket>& in, const L3Address &neigh, uint8_t directlink, int16_t hna_buff_len, BatmanIf *if_incoming, const simtime_t &curr_time)
{
    ...
    for ( ; it!=forw_list.end(); it++) {
        ...
        if (aggregation_enabled) {
            ...
            if ((forw_node_pos->send_time < send_time) &&
                (forw_node_pos->pack_buff->getDataLength() + in->getChunkLength() <= B(MAX_AGGREGATION_BYTES))) {
                auto bat_packet = forw_node_pos->pack_buff->peekAtFront<BatmanPacket>();
    ...
    const auto& bat_packet = staticPtrCast<BatmanPacket>(in->dupShared());
    ...
    if (forw_node_aggregate == nullptr) {
        ...
        forw_node_new->pack_buff = new Packet(bat_packet->getName());
        forw_node_new->pack_buff->insertAtBack(bat_packet);
...
void Batman::send_outstanding_packets(const simtime_t &curr_time)
{
    ...
    while (true) {
        ...
        auto bat_packet = forw_node->pack_buff->peekAtFront<BatmanPacket>();
        ...
        if (((directlink) && (bat_packet->getTtl() == 1)) ||
            ...
            if (send_udp_packet(forw_node->pack_buff->dup(), forw_node->pack_buff_len, forw_node->if_incoming->broad, par("UdpPort"), forw_node->if_incoming) < 0)
        ...
        for (auto & elem : if_list) {
            ...
            auto packetSend = new Packet();

            auto pktAux = forw_node->pack_buff->dup();
            pktAux->trim();

            if (pktAux->getDataLength() == b(0))
                throw cRuntimeError("Batman packet erroneous, not CHUNK of type  BatmanPacket");

            if (!pktAux->hasAtFront<BatmanPacket>())
              throw cRuntimeError("Batman packet erroneous, not CHUNK of type  BatmanPacket");

            do {
                auto bat_packet = pktAux->removeAtFront<BatmanPacket>();
...
//                    addr_to_string(bat_packet->orig, orig_str, ADDR_STR_LEN);
                ...
                if ((batman_if->wifi_if) && (!forw_node->own) && (forw_node->if_incoming == batman_if))
                    bat_packet->setTq((bat_packet->getTq() * (TQ_MAX_VALUE - (2 * hop_penalty))) / (TQ_MAX_VALUE));
                ...
                packetSend->insertAtBack(bat_packet);
            } while(pktAux->getDataLength() > b(0) && pktAux->hasAtFront<BatmanPacket>());

            delete pktAux;

            if (send_udp_packet(packetSend, forw_node->pack_buff_len, batman_if->broad, batman_if->udp_send_sock, batman_if) < 0)
