...
void Batman::update_routes(OrigNode *orig_node, NeighNode *neigh_node, const HnaElement *hna_recv_buff, int16_t hna_buff_len)
{
...
int Batman::isBidirectionalNeigh(OrigNode *orig_node, OrigNode *orig_neigh_node,const Ptr<BatmanPacket>& in, const simtime_t &recv_time, BatmanIf *if_incoming)
{
...
uint8_t Batman::count_real_packets(const BatmanPacket* in, const L3Address &neigh, BatmanIf *if_incoming)
{
    OrigNode *orig_node;
...
*/
void Batman::parseIncomingPacket(L3Address neigh, BatmanIf *if_incoming, Packet *msg)
{
    OrigNode *orig_neigh_node, *orig_node;
    simtime_t curr_time;
    //char orig_str[ADDR_STR_LEN], neigh_str[ADDR_STR_LEN], ifaddr_str[ADDR_STR_LEN], prev_sender_str[ADDR_STR_LEN];
...

    {
        ...
        while ((msg->getDataLength() > b(0)) && msg->hasAtFront<BatmanPacket>()) {
            ...
            has_directlink_flag = (batmanPacketAux.getFlags() & DIRECTLINK ? 1 : 0);

            EV_DETAIL << "Received BATMAN packet via NB: " << neigh << ", IF: " << if_incoming->dev
                    << " (from OG: " << batmanPacketAux.getOrig() << ", via old OG: " << batmanPacketAux.getPrevSender()
                    << ", seqno " << batmanPacketAux.getSeqNumber() << ", tq " << (unsigned)(batmanPacketAux.getTq())
                    << ", TTL " << (unsigned)(batmanPacketAux.getTtl()) << ", V " << (unsigned)(batmanPacketAux.getVersion())
                    << ", IDF " << (unsigned)(has_directlink_flag) << ")\n";

            hna_buff_len = batmanPacketAux.getHnaMsgArraySize() * BATMAN_HNA_MSG_SIZE;
            unsigned int  hnaLen = batmanPacketAux.getHnaMsgArraySize();
            auto hna_recv_buff = hnaLen > 0 ? &batmanPacketAux.getHnaMsg(0) : nullptr;

            {
                ...
                if (isLocalAddress(batmanPacketAux.getOrig()))
                    is_my_orig = 1;

                if (isMulticastAddress(neigh))
                    is_broadcast = 1;

                if (isLocalAddress(batmanPacketAux.getPrevSender()))
                    is_my_oldorig = 1;
            }

            if (batmanPacketAux.getGatewayFlags() != 0)
                EV_DETAIL << "Is an internet gateway (class " << batmanPacketAux.getGatewayFlags() << ") \n";

            if (batmanPacketAux.getVersion() != 0) {
                EV_INFO << "Drop packet: incompatible batman version "<< (unsigned)(batmanPacketAux.getVersion()) << endl;
            ...
            if (is_my_orig) {
                ...
                bool sameIf = false;
                if (if_incoming->dev->getProtocolData<Ipv4InterfaceData>()->getIPAddress() == batmanPacketAux.getOrig().toIpv4())
                    sameIf = true;

                if ((has_directlink_flag) && (sameIf) && (batmanPacketAux.getSeqNumber() - if_incoming->seqno + 2 == 0)) {
                ...
                msg->popAtFront<BatmanPacket>();
                continue;
            }

            if (batmanPacketAux.getTq() == 0) {
                count_real_packets(&batmanPacketAux, neigh, if_incoming);
            ...
            is_duplicate = count_real_packets(&batmanPacketAux, neigh, if_incoming);

            orig_node = get_orig_node(batmanPacketAux.getOrig());

            /* if sender is a direct neighbor the sender ip equals originator ip */
            orig_neigh_node = (batmanPacketAux.getOrig() == neigh ? orig_node : get_orig_node(neigh));

            /* drop packet if sender is not a direct neighbor and if we no route towards it */
            if ((batmanPacketAux.getOrig() != neigh) && (orig_neigh_node->router == nullptr)) {
            ...
            orig_node->totalRec++;
            msg->trimFront();
            auto &pktAux = msg->removeAtFront<BatmanPacket>();
            is_bidirectional = isBidirectionalNeigh(orig_node, orig_neigh_node, pktAux, curr_time, if_incoming);
            msg->insertAtFront(pktAux);


            /* update ranking if it is not a duplicate or has the same seqno and similar ttl as the non-duplicate */
            if ((is_bidirectional) && ((!is_duplicate) ||
                 ((orig_node->last_real_seqno == batmanPacketAux.getSeqNumber()) &&
                 (orig_node->last_ttl - 3 <= batmanPacketAux.getTtl()))))
                update_orig(orig_node, &batmanPacketAux, neigh, if_incoming, hna_recv_buff, hna_buff_len, is_duplicate, curr_time);

            /* is single hop (direct) neighbour */
            if (batmanPacketAux.getOrig() == neigh) {
                /* mark direct link on incoming interface */
                schedule_forward_packet(orig_node,dynamicPtrCast<BatmanPacket>(msg->peekAtFront<BatmanPacket>()->dupShared()), neigh, 1, hna_buff_len, if_incoming, curr_time);
            ...
            schedule_forward_packet(orig_node, dynamicPtrCast<BatmanPacket>(msg->peekAtFront<BatmanPacket>()->dupShared()), neigh, 0, hna_buff_len, if_incoming, curr_time);
            msg->popAtFront<BatmanPacket>();
        }
