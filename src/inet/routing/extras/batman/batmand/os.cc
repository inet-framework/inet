...
int8_t Batman::send_udp_packet(Packet *packet_buff, int32_t packet_buff_len, const L3Address & destAdd, int32_t send_sock, BatmanIf *batman_if)
{
    ...
    if (batman_if)
        sendToIp(packet_buff, par("UdpPort"), destAdd, par("UdpPort"), 1, par("broadcastDelay").doubleValue(), L3Address(batman_if->dev->getProtocolData<Ipv4InterfaceData>()->getIPAddress()));
    else
        sendToIp(packet_buff, par("UdpPort"), destAdd, par("UdpPort"), 1, par("broadcastDelay").doubleValue(), L3Address());
    return 0;
}
...
int Batman::add_del_interface_rules(int8_t rule_action)
{
    ...
    for (int i=0; i<getNumInterfaces(); i++)
    {
        ...
        L3Address addr(ifr->getProtocolData<Ipv4InterfaceData>()->getIPAddress());
        L3Address netmask(ifr->getProtocolData<Ipv4InterfaceData>()->getNetmask());
        uint8_t mask = ifr->getProtocolData<Ipv4InterfaceData>()->getNetmask().getNetmaskLength();
