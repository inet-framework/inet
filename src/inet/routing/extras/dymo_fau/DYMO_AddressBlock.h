...
{
  protected:
    L3Address address; /**< The IPv4 address of an additional node that can be reached via the DYMO router adding this information.  Each AdditionalNode.Address must have an associated Node.SeqNum in the address TLV block. */
  ...
  public:
    ...
    std::string str() const {
        ...
        if (hasAddress()) {
            os += "Address=" + getAddress().str();
            cModule *m = L3AddressResolver().findHostWithAddress(getAddress());
            if (m)
                os += " (" + std::string(m->getFullName()) + ")";
        }

        os += ", ";

        if (hasPrefix()) {
            os += "Prefix=" + std::to_string(getPrefix());
        }

        os += ", ";

        if (hasDist()) {
            os += "Dist=" + std::to_string(getDist());
        }

        os += ", ";

        if (hasSeqNum()) {
            os += "SeqNum=" + std::to_string(getSeqNum());
        }
