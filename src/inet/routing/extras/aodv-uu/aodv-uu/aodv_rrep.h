...
#ifdef OMNETPP
Ptr<RREP> rrep_create(u_int8_t flags,
                  u_int8_t prefix,
                  u_int8_t hcnt,
...
Ptr<RREP_ack>  rrep_ack_create();
AODV_ext *rrep_add_ext(Ptr<RREP> rrep, int type, unsigned int offset,
                       int len, char *data);
void rrep_send(Ptr<RREP> rrep, rt_table_t * rev_rt, rt_table_t * fwd_rt, int size, double delay = 0);
void rrep_forward(Ptr<RREP> rrep, int size, rt_table_t * rev_rt,
                  rt_table_t * fwd_rt, int ttl);
void rrep_process(Ptr<RREP> rrep, int rreplen, struct in_addr ip_src,
                  struct in_addr ip_dst, int ip_ttl, unsigned int ifindex);
void rrep_ack_process(Ptr<RREP_ack> rrep_ack, int rreplen, struct in_addr ip_src,
