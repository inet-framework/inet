...
Ptr<RERR> NS_CLASS rerr_create(u_int8_t flags,struct in_addr dest_addr,
...
void NS_CLASS rerr_add_udest(Ptr<RERR> &rerr,struct in_addr udest,
...
void NS_CLASS rerr_process(Ptr<RERR> rerr, int rerrlen,struct in_addr ip_src,
                           struct in_addr ip_dst)
{
    Ptr<RERR> new_rerr = nullptr;
    RERR_udest *udest;
    rt_table_t *rt;
...
#ifdef OMNETPP
                if (i > 0) {
                    aodv_socket_send(p->dup(), dest,RERR_CALC_SIZE(new_rerr), 1, &DEV_NR(i));
                }
                else
#endif
                    aodv_socket_send(p, dest,RERR_CALC_SIZE(new_rerr), 1, &DEV_NR(i));
                numInterfaces--;
            }
