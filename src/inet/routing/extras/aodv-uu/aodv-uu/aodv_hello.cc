...
long NS_CLASS hello_jitter()
{
    if (hello_jittering)
    {
        ...
        return (long) (((float) Random::integer(RAND_MAX + 1) / RAND_MAX - 0.5)
                       * JITTER_INTERVAL);
#else
        return (long) (((double) intuniform(0, RAND_MAX) / (double)RAND_MAX - 0.5) * JITTER_INTERVAL);
#endif
#else
    ...
    if (time_diff >= HELLO_INTERVAL)
    {
        ...
        for (i = 0; i < MAX_NR_INTERFACES; i++)
        {
...
#endif
            auto rrep = rrep_create(flags, 0, 0, DEV_NR(i).ipaddr,
                               this_host.seqno,
                               DEV_NR(i).ipaddr,
            ...
            if (unidir_hack)
            {
                ...
                rrep->setChunkLength(B(msg_size));
#else
            ...
            aodv_socket_send(p, dest, msg_size, 1, &DEV_NR(i),delay);
#else
...
void NS_CLASS hello_process(Ptr<RREP> hello, int rreplen, unsigned int ifindex)
{
