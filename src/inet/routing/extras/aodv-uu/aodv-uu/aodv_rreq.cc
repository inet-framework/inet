...
Ptr<RREQ> NS_CLASS rreq_create(u_int8_t flags,struct in_addr dest_addr,
...
AODV_ext *rreq_add_ext(Ptr<RREQ> rreq, int type, unsigned int offset,
                       int len, char *data)
{
...
#ifdef OMNETPP
        rreq->ttl = ttl;
        auto pkt = new Packet("Aodv RREQ",rreq);
        aodv_socket_send(pkt, dest, RREQ_SIZE, 1, &DEV_NR(i),delay);
        totalRreqSend++;
#else
...
void NS_CLASS rreq_forward(Ptr<RREQ> rreq, int size, int ttl)
{
...
void NS_CLASS rreq_process(Ptr<RREQ> rreq, int rreqlen, struct in_addr ip_src,
                           struct in_addr ip_dst, int ip_ttl,
                           unsigned int ifindex)
{

    AODV_ext *ext=nullptr;
    Ptr <RREP> rrep = nullptr;
