...
Ptr<RREP> NS_CLASS rrep_create(u_int8_t flags,
                           u_int8_t prefix,
                           u_int8_t hcnt,
...
Ptr<RREP_ack> NS_CLASS rrep_ack_create()
{
...
void NS_CLASS rrep_ack_process(Ptr<RREP_ack> rrep_ack, int rrep_acklen,
...
AODV_ext *NS_CLASS rrep_add_ext(Ptr<RREP> rrep, int type, unsigned int offset,
                                int len, char *data)
{
...
#endif
    rrep->ttl=MAXTTL;
    Packet *p = new Packet("Aodv RREP");
    p->insertAtFront(rrep);
    if (delay > 0)
        aodv_socket_send(p, rev_rt->next_hop, size, 1,
...
void NS_CLASS rrep_forward(Ptr<RREP> rrep, int size, rt_table_t * rev_rt,
...
#else
    auto rrep_new = Ptr<RREP> (rrep->dup());
    rrep_new->hcnt = fwd_rt->hcnt;
    rrep_new->hopfix = fwd_rt->hopfix;
    totalRrepSend++;
    rrep_new->ttl=ttl;
    Packet *p = new Packet("Aodv RREP");
    p->insertAtFront(rrep_new);
    aodv_socket_send(p, rev_rt->next_hop, size, 1,
                     &DEV_IFINDEX(rev_rt->ifindex));
#endif
    precursor_add(fwd_rt, rev_rt->next_hop);
...
void NS_CLASS rrep_process(Ptr<RREP> rrep, int rreplen, struct in_addr ip_src,
                           struct in_addr ip_dst, int ip_ttl,unsigned int ifindex)
{
    ...
    if (rrep->a)
    {
        auto rrep_ack = rrep_ack_create();
        totalRrepAckSend++;
        rrep_ack->ttl=MAXTTL;
        auto pkt = new Packet("Aodv ACK");
        pkt->insertAtBack(rrep_ack);


        aodv_socket_send(pkt, fwd_rt->next_hop,
    ...
    if (isLocalAddress (rrep_orig.s_addr))
    {
        ...
        if (pre_repair_flags & RT_REPAIR)
        {
            if (fwd_rt->hcnt > pre_repair_hcnt)
            {
                ...
                if (fwd_rt->nprec)
                {
                    auto rerr = rerr_create(rerr_flags, fwd_rt->dest_addr,
                                       fwd_rt->dest_seqno);
                    rerr->ttl=1;
                    auto pkt = new Packet("Aodv RERR");
                    pkt->insertAtBack(rerr);
                    aodv_socket_send(pkt, dest,
