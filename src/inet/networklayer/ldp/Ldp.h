...
// FEC's address family isn't known at compile time (RFC 7552 IPv6 FECs) --
// ldpFecTlvBytes() returns exactly LDP_FEC_TLV_BYTES for an IPv4 FEC, so these constants
// remain the correct (and byte-identical) value whenever no IPv6 FEC is involved.
...
// Dual-stack (RFC 7552): the size of a FecTlv on the wire depends on its address
// family -- LDP_FEC_TLV_BYTES (12 bytes total) for an IPv4 FEC, or 24 bytes (4-byte TLV
// header + 1(elem type) + 2(addr family) + 1(prelen) + 16(prefix)) for an IPv6 one. Used
// both to size outgoing messages (Ldp::sendMappingRequest/sendMapping/sendNotify) and by
// LdpPacketSerializer to write the matching on-wire TLV value length, so the two can
// never drift apart.
...
class INET_API Ldp : public RoutingProtocolBase, public TcpSocket::BufferingCallback, public UdpSocket::ICallback, public cListener
{
  public:
    ...
    struct Fec {
        ...
        // FEC value. Dual-stack (RFC 7552): holds either an Ipv4Address or an Ipv6Address
        // prefix -- see rebuildFecList() for how an IPv6 route becomes one of these.
        L3Address addr;
        int length;
...
//        std::string nextHopInterface
  ...
  protected:
    ...
    // Dual-stack: optional -- null on a plain IPv4-only router (no Ipv6RoutingTable
    // submodule at all, e.g. hasIpv6=false), in which case rebuildFecList()'s IPv6 loop
    // is a complete no-op. See routingTableModule6 in Ldp.ned.
  ...
  protected:
    ...
    // Dual-stack: returns the DIRECTLY CONNECTED neighbor's address (gateway
    // unspecified, or gateway==destination -- this model's convention for an
    // ARP-resolvable host route) reachable via the given local interface, or the
    ...
    // Ipv4Address even for an IPv6 FEC. Deliberately ignores INDIRECT (multi-hop)
    // routes sharing the same interface (e.g. ones installed once Ted/LinkStateRouting
    ...
    // the FEC value itself, dual-stack (IPv4 or IPv6).
    ...
    // Removes the LIB entry an fecUp binding's label stands for. Must be called
    // BEFORE the binding is dropped from fecUp: that binding is the only record of
    // which LIB entry this module owns, so an entry outliving it can never be found
    // again -- it would keep swapping traffic that arrives with a label no longer
    // advertised to anyone. An implicit-null advertisement never allocated an entry,
    // and under DU independent control a label may have been advertised (reserved via
    // LibTable::allocateLabel()) but never installed -- removeLibEntryIfExists()
    // tolerates both.
    virtual void removeUpstreamLibEntry(const FecBinding& binding);
  ...
  public:
    ...
    /**
     * Ingress classification decision for an incoming Ipv4 or Ipv6 datagram:
     * longest-prefix match over fecList, followed by a fecDown lookup for the matched
     * FEC, plus the LDP/OSPF port skip-rules (Ipv4 only -- LDP's own control-plane
     * traffic never runs over Ipv6 in this model). Called by the sibling ~LdpClassifier
     * module, which implements IIngressClassifier and delegates to this method rather
     * than maintaining its own bind-time FEC/label table -- see LdpClassifier.h for why.
     */
  ...
  protected:
    ...
    // reply to a locally-adjacent peer's Hello) leaves both bits unset.
