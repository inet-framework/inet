...
// Dual-stack: every LibTable::installLibEntry()/installReservedLabel() call this
// module makes must tag the LIB entry with the correct payload protocol for the FEC
// it serves -- LibTable defaults to &Protocol::ipv4, which silently mislabels an
// IPv6 FEC's entries and crashes Mpls::popLabel()'s caller the moment that entry is
// used (it reinterprets the popped IPv6 bytes as an Ipv4Header).
...
void Ldp::initialize(int stage)
{
    ...
    if (stage == INITSTAGE_LOCAL) {
        ...
        // Dual-stack: optional -- a plain IPv4-only router (hasIpv6=false, the
        // default) has no Ipv6RoutingTable submodule at all, so this reference
        // stays null and rebuildFecList()'s IPv6 loop never runs.
...
{
    for (auto& elem : myPeers) {
        // a stop/crash takes an OPERATIONAL session down as surely as any other
        // teardown path does; without this the sessionUp/sessionDown pair diverges
        if (elem.state == peer_info::OPERATIONAL)
            emit(sessionDownSignal, (long)elem.peerIP.getInt());
        cancelAndDelete(elem.timeout);
        cancelAndDelete(elem.keepAliveSendTimer);
        cancelAndDelete(elem.sessionHoldTimer);
    }
    ...
    for (auto *m : retryMsgs)
        cancelAndDelete(m);
    retryMsgs.clear();
    // drop label bindings so a restart does not resume from stale state -- including
    // the LIB entries they stand for: LibTable has no lifecycle handling of its own,
    // so entries left behind here would keep forwarding labeled traffic per pre-stop
    // state and leak another full set on every stop/start cycle
    for (auto& binding : fecUp)
        removeUpstreamLibEntry(binding);
    fecUp.clear();
    fecDown.clear();
    fecList.clear();
    pending.clear();
        ...
        if (ER) {
            if (advertiseImplicitNull) {
                ...
                if (uit->label != IMPLICIT_NULL_LABEL) {
                    // We were a transit LSR for this FEC until now, so the peer holds
                    // a real label of ours whose LIB entry still swaps toward the old
                    // next hop. Unlike every other branch here, the label the peer
                    // must use actually changes -- withdraw the old one (and its LIB
                    // entry, which nothing could find any more once 'label' reads 3)
                    // and advertise the new mapping (RFC 5036 Section 2.6.1).
                    EV_INFO << "no longer transit for fec addr=" << oldItem.addr << " length=" << oldItem.length
                            << ": withdrawing label=" << uit->label << " from " << uit->peer
                            << " and advertising the implicit null label instead" << endl;
                    sendMapping(LABEL_WITHDRAW, uit->peer, uit->label, oldItem.addr, oldItem.length);
                    lt->removeLibEntryIfExists(uit->label);
                    uit->label = IMPLICIT_NULL_LABEL;
                    sendMapping(LABEL_MAPPING, uit->peer, uit->label, oldItem.addr, oldItem.length);
                }
...
void Ldp::rebuildFecList()
{
    ...
    // Dual-stack (RFC 7552): IPv6 FEC advertisement over this (IPv4-transport) LDP
    // session. rt6 is null on a plain IPv4-only router (no Ipv6RoutingTable
    // submodule at all -- see routingTableModule6 in Ldp.ned), in which case this
    // loop is a complete no-op and fecList is entirely unaffected.
    if (rt6) {
...
void Ldp::removeUpstreamLibEntry(const FecBinding& binding)
{
    if (binding.label == IMPLICIT_NULL_LABEL)
        return;

    EV_DETAIL << "removing entry inLabel=" << binding.label << " from LIB (if it was ever installed)" << endl;
    lt->removeLibEntryIfExists(binding.label);
}
...
void Ldp::processHelloTimeout(cMessage *msg)
{
    ...
    // A dead Hello adjacency takes the session with it, so report it exactly as the
    // other two teardown paths do (handleTcpConnectionDown/processSessionHoldTimeout).
    // abort() below invokes no callback, so nothing else ever would -- and this is the
    // path a link failure that TCP has not itself noticed reaches first.
...
void Ldp::removePeerBindings(Ipv4Address peerIP)
{
    ...
    for (auto uit = fecUp.begin(); uit != fecUp.end();) {
        ...
        // the label we advertised to this peer means nothing once its session is
        // gone; leaving its LIB entry behind would keep swapping traffic that still
        // arrives labeled with it toward a next hop this teardown may well have
        // invalidated (and nothing could ever find the entry to remove afterwards)
        removeUpstreamLibEntry(*uit);
...
Ldp::FecVector::iterator Ldp::findFecEntry(FecVector& fecs, L3Address addr, int length)
{
    auto it = fecs.begin();
    for (; it != fecs.end(); it++) {
        // Dual-stack: fecs may hold a MIX of IPv4 and IPv6 entries -- skip cross-family
        // comparisons before matching (L3Address's own operator== already handles this
        // safely, but skipping first avoids wasting a comparison and keeps this in line
        // with the same idiom used elsewhere, e.g.
...
void Ldp::processLABEL_WITHDRAW(Ptr<const LdpPacket>& ldpPacket)
{
    ...
    // 'reply' is a copy of the incoming withdraw and so still carries ITS
    // sender's lsrId, not our own -- the receiving peer's getLsrId() therefore
    // identifies the wrong LSR for this message. Harmless today (routing uses
    // fromIP via sendToPeer, and the mismatch only degrades this reply's own
    // peer-identification on the far end to a silent no-op), but worth a
    // follow-up: reply->setLsrId(rt->getRouterId()).
...
{
    // Dual-stack: the packet's native protocol (ipv4 or ipv6, both offered to
    // classifiers by Mpls -- see Mpls::processPacketFromL3/processPacketFromL2)
    // determines how the destination address is extracted; the IPv4 branch below
    // also applies the LDP/OSPF control-plane port skip-rules -- those rules are
    // IPv4-transport-only in this model (LDP's own Hello/session traffic never runs
    // over IPv6), so there is no IPv6 equivalent to replicate.
