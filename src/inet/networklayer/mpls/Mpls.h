...
class INET_API Mpls : public SimpleModule, public DefaultProtocolRegistrationListener, public IInterfaceRegistrationListener
{
  ...
  protected:
    ...
    // the bottom-of-stack label is popped; defaults to Ipv4.
    void popLabel(Packet *packet, const Protocol *payloadProtocol = &Protocol::ipv4);

    // Single dispatch point for reading the packet's current native hop count --
    // Ipv4Header::TimeToLive or Ipv6Header::HopLimit, whichever the packet's
    // PacketProtocolTag currently names. Used by computePushTtl().
    ...
    // re-inserted, keeping the update to a single remove/insert.
    void setNativeHopCount(Packet *packet, uint8_t hopCount, bool writeDscp = false, uint8_t dscp = 0);

    // Read-side dispatch mirroring getNativeHopCount(), but for Dscp -- both
    // Ipv4Header and Ipv6Header expose getDscp() with the same shape. Used by
    // computePushTc().
    ...
    // Exceeded. payloadProtocol is what this LIB entry's label ultimately carries;
    // defaults to Ipv4 for source compatibility.
    ...
    // here). protocol defaults to Ipv4 for source compatibility.
    ...
    // payloadProtocol is forwarded to popLabel() and handleTtlExpiry() for the
    // POP_OPER/expiry cases; defaults to Ipv4 since the ingress call site
    // (tryLabelAndForwardDatagram()) has no LIB entry to ask.
