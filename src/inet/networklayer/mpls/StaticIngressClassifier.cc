...
{
    // Dual-stack: the packet's native protocol (ipv4 or ipv6, both offered to classifiers by
    // Mpls -- see Mpls::processPacketFromL3/processPacketFromL2) is already recorded on its
    // PacketProtocolTag; peekNetworkProtocolHeader() dispatches to the right concrete header
    // type and returns it through the common NetworkHeaderBase interface, so
...
void StaticIngressClassifier::readTableFromXML(const cXMLElement *fectable)
{
    ...
    for (auto& elem : list) {
        ...
        // Dual-stack: exactly one of the old 'netmask' (IPv4 dotted, kept for backward
        // compatibility -- e.g. examples/mpls/sr/policies.xml's sibling SrPolicy format) or the
        // new 'prefixLength' (a plain integer, usable for either family, mandatory for an IPv6
        // dest since IPv6 has no netmask-string convention) attribute may be given.
