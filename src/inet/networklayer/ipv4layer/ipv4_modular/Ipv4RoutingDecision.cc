...
#include "inet/networklayer/ipv4layer/ipv4_modular/Ipv4RoutingDecision.h"
...
const NetworkInterface *Ipv4RoutingDecision::getSourceInterface(Packet *packet)
{
    const auto& tag = packet->findTag<InterfaceInd>();
    return tag != nullptr ? ift->getInterfaceById(tag->getInterfaceId()) : nullptr;
}

const NetworkInterface *Ipv4RoutingDecision::getDestInterface(Packet *packet)
{
    const auto& tag = packet->findTag<InterfaceReq>();
    return tag != nullptr ? ift->getInterfaceById(tag->getInterfaceId()) : nullptr;
}

Ipv4Address Ipv4RoutingDecision::getNextHop(Packet *packet)
{
...
void Ipv4RoutingDecision::routeUnicastPacket(Packet *packet)
{
...
void Ipv4RoutingDecision::forwardMulticastPacket(Packet *packet)
{
