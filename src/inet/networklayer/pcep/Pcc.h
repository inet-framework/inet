 ...
 * discovery phase. For stateless path computation (RFC 5440 Section 6.5/6.6) it offers
...
class INET_API Pcc : public RoutingProtocolBase, public TcpSocket::BufferingCallback
{
  ...
  protected:
    // One entry per path computation request this Pcc has sent to the PCE and not yet
    // handed back to its caller (~RsvpTe) -- see requestPathComputation().
    ...
    // result is consumed by requestPathComputation() -- stateless PCEP (RFC 5440
    // Section 6.5/6.6) keeps nothing that outlives a single request/reply/consume cycle.
    struct PceRequest {
    ...
    // RFC 8231 stateful delegation: bookkeeping for one delegated LSP this Pcc has
    // reported (or is about to report) to the PCE -- see reportLsp(), called
    ...
    // PCEP has no other identity to key by, whereas stateful delegation has a real,
    // RFC-defined identity (PLSP-ID) to use instead. The two mechanisms
    ...
    // RFC 8231 Section 5.2 state timeout -- how long this PCC waits, after the PCEP
    // session is lost, for the PCE to come back before reverting every delegated LSP
    // to local control (see Pcc.ned's doc comment and processStateTimeout()).
    simtime_t stateTimeout;
    cMessage *stateTimeoutTimer = nullptr;

    // An optional back-reference to the co-located ~RsvpTe this Pcc's delegated LSPs
    // actually belong to -- needed both to apply an incoming PCUpd
    ...
    // stateless deployment never sets it and never touches any of the
    // stateful-delegation code.
  ...
  protected:
    ...
    // with delegated LSPs still outstanding; tell ~RsvpTe to fall back
  ...
  public:
    // RFC 5440 Section 6.5/6.6: the bridge between ~RsvpTe's synchronous ingress path
    // computation call and this module's asynchronous PCReq/PCRep TCP exchange.
    // Called by ~RsvpTe::createIngressPSB() in place of its own local
    ...
    //   here, matching stateless PCEP semantics) and returns COMPUTED
    ...
    //   PATH_RETRY handling: "no path yet" and "awaiting a reply" are treated
    //   identically);
    ...
    // RFC 8231 Section 6.1: called by ~RsvpTe whenever a delegated ingress LSP it
    // manages comes up (up=true, a real ERO), changes route via an internal
    ...
    // back up -- is not modeled (a documented simplification).
