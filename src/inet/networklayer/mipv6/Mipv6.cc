...
void Mipv6::handleMessageWhenUp(cMessage *msg)
{
    ...
    // MIPv6 related mobility message
        ...
        if (packet->getTag<PacketProtocolTag>()->getProtocol() == &Protocol::mobileipv6) {
            ...
            throw cRuntimeError("Received packet '%s' (%s) with a protocol other than mobileipv6", packet->getName(), packet->getClassName());
    }
...
void Mipv6::initiateMipv6Protocol(NetworkInterface *ie, const Ipv6Address& CoA)
{
    ...
    Ipv6Address HoA = ie->getProtocolData<Mipv6InterfaceData>()->getMNHomeAddress();
...
void Mipv6::sendPeriodicBU(cMessage *msg)
{
    ...
    if (!(buIfEntry->ackTimeout < ie->getProtocolData<Mipv6InterfaceData>()->_getMaxBindAckTimeout())) {
        EV_DETAIL << "Crossed maximum BINDACK timeout...resetting to predefined maximum." << endl;
        buIfEntry->ackTimeout = ie->getProtocolData<Mipv6InterfaceData>()->_getMaxBindAckTimeout();
    }
...
void Mipv6::createAndSendBUMessage(const Ipv6Address& dest, NetworkInterface *ie, const uint buSeq, const uint lifeTime, const int bindAuthData)
{
    ...
    bu->setHomeRegistrationFlag(dest == ie->getProtocolData<Mipv6InterfaceData>()->getHomeAgentAddress());
        ...
        NetworkInterface *ie, const simtime_t sendTime)
{
    ...
    // to point to the struct where i am globally recording the statistics for sent time and next sent time for the BU:
    ...
    if (rt6->isMobileNode()) {
        ...
        if (ba->getStatus() < 128) {
            ...
            if (ba->getLifetime() == 0) { // BAck to deregistration BU
                if (baSource == ie->getProtocolData<Mipv6InterfaceData>()->getHomeAgentAddress()) {
                    /*11.5.4
                ...
                if (entry->BAck == false && entry->destAddress == ie->getProtocolData<Mipv6InterfaceData>()->getHomeAgentAddress()) { // BA from HA
                    ...
                    for (itCNList = cnList.begin(); itCNList != cnList.end(); itCNList++) { // run an iterator through the CN map
                        ...
                        triggerRouteOptimization(cnDest, ie->getProtocolData<Mipv6InterfaceData>()->getMNHomeAddress(), ie);
                    }
                }
...
bool Mipv6::validateBAck(Packet *packet, const BindingAcknowledgement& ba)
{
    ...
    if (cnAddress != ie->getProtocolData<Mipv6InterfaceData>()->getHomeAgentAddress()) { // BAck comes from a CN and not from the HA
...
{
    EV_DETAIL << "Test init timer created\n";
    ...
    /*o  Otherwise, the mobile node should use the specified value of
           INITIAL_BINDACK_TIMEOUT for the initial retransmission timer.*/
    tiIfEntry->ackTimeout = ie->getProtocolData<Mipv6InterfaceData>()->_getInitialBindAckTimeout();
...
void Mipv6::sendTestInit(cMessage *msg)
{
    ...
    if (!(tiIfEntry->ackTimeout < ie->getProtocolData<Mipv6InterfaceData>()->_getMaxBindAckTimeout()))
        tiIfEntry->ackTimeout = ie->getProtocolData<Mipv6InterfaceData>()->_getMaxBindAckTimeout();
...
void Mipv6::resetBUIfEntry(const Ipv6Address& dest, int interfaceID, simtime_t retransmissionTime)
{
    ...
    entry->ackTimeout = entry->ifEntry->getProtocolData<Mipv6InterfaceData>()->_getInitialBindAckTimeout();
...
void Mipv6::processHoTIMessage(Packet *inPacket, const Ptr<const HomeTestInit>& homeTestInit)
{
    ...
    EV_INFO << "HoT message formed, appending control info\n";
...
void Mipv6::processCoTIMessage(Packet *inPacket, const Ptr<const CareOfTestInit>& coti)
{
    ...
    EV_INFO << "CoT message formed, appending control info\n";
...
void Mipv6::processHoTMessage(Packet *inPacket, const Ptr<const HomeTest>& homeTest)
{
    if (!validateHoTMessage(inPacket, *homeTest)) {
        ...
        createHomeTokenEntryExpiryTimer(srcAddr, ift->getInterfaceById(interfaceID),
                simTime() + ie->getProtocolData<Mipv6InterfaceData>()->_getMaxTokenLifeTime());
        ...
        const Ipv6Address& entry, const Ipv6Address& exit, const Ipv6Address& destTrigger)
{
    ...
    if ((destTrigger == Ipv6Address::UNSPECIFIED_ADDRESS) && (noOfNonSplitTunnels == 1))
        throw cRuntimeError("Cannot create a second non-split tunnel (entry=%s exit=%s); only one non-split tunnel is supported", entry.str().c_str(), exit.str().c_str());
...
Mipv6::TimerIfEntry *Mipv6::getTimerIfEntry(Key& key, int timerType)
{
    ...
    if (pos != transmitIfList.end()) {
        ...
        if (TestInitTransmitIfEntry *testInitIfEntry = dynamic_cast<TestInitTransmitIfEntry *>(pos->second)) {
            cancelAndDelete(testInitIfEntry->timer);
        ...
        switch (timerType) {
            ...
            default:
                throw cRuntimeError("Unknown timer type %d", timerType);
                break;
        }
...
void Mipv6::cancelEntries(int interfaceId, Ipv6Address& CoA)
{
    ...
    // ...first for the HA
    Ipv6Address HA = ie->getProtocolData<Mipv6InterfaceData>()->getHomeAgentAddress();
...
void Mipv6::createBRRTimer(const Ipv6Address& brDest, NetworkInterface *ie, const uint scheduledTime)
{
    ...
    if (pos != transmitIfList.end()) {
        ...
        if ((brIfEntry = dynamic_cast<BrTransmitIfEntry *>(pos->second))) {
            cancelAndDelete(brIfEntry->timer);
        }
        else
            throw cRuntimeError("Existing timer entry for %s is not a BrTransmitIfEntry", brDest.str().c_str());
    }
    else {
        // we do not yet have an entry -> create a new one
    ...
    EV_DETAIL << "BRR timer created for sim time " << simTime() + scheduledTime << "s\n";
}

void Mipv6::sendPeriodicBRR(cMessage *msg)
{
    EV_INFO << "Sending periodic BRR message\n";
...
void Mipv6::handleBULExpiry(cMessage *msg)
{
    ...
    if (simTime() < entry->bindingExpiry) {
        ...
        if (bulExpIfEntry->dest == ie->getProtocolData<Mipv6InterfaceData>()->getHomeAgentAddress())
...
void Mipv6::handleTokenExpiry(cMessage *msg)
{
    ...
    if (tokenExpIfEntry->tokenType == KEY_CTOKEN_EXP) {
        ...
        bul->resetCareOfToken(tokenExpIfEntry->cnAddr, tokenExpIfEntry->ifEntry->getProtocolData<Mipv6InterfaceData>()->getMNHomeAddress());
    }
    else if (tokenExpIfEntry->tokenType == KEY_HTOKEN_EXP) {
        EV_INFO << "Home keygen token for CN=" << tokenExpIfEntry->cnAddr << " expired";
        bul->resetHomeToken(tokenExpIfEntry->cnAddr, tokenExpIfEntry->ifEntry->getProtocolData<Mipv6InterfaceData>()->getMNHomeAddress());
    }
    else
        throw cRuntimeError("Unknown token type %d for CN %s (expected KEY_CTOKEN_EXP or KEY_HTOKEN_EXP)", tokenExpIfEntry->tokenType, tokenExpIfEntry->cnAddr.str().c_str());
