...
const char *SctpAssociation::eventName(int32_t event)
{
#define CASE(x)    case x: \
        s = (char *)#x + 7; break
    const char *s = "unknown";
...
const char *SctpAssociation::indicationName(int32_t code)
{
#define CASE(x)    case x: \
        s = (char *)#x + 7; break
    const char *s = "unknown";
...
uint16_t SctpAssociation::chunkToInt(const char *type)
{
    EV_STATICCONTEXT;
...
//    msg->setControlInfo(availableIndication);
    // the available indication belongs to the listening socket, and this forked
    // association has no callback registered until the application accepts it
    auto listeningAssoc = sctpMain->getAssoc(listeningAssocId);
    if (listeningAssoc == nullptr || listeningAssoc->callback == nullptr)
        throw cRuntimeError("No callback registered for listening SCTP socket, socketId = %d", listeningAssocId);
    listeningAssoc->callback->handleAvailable(msg);
}
...
//    msg->setControlInfo(establishIndication);
    // a forked association has no callback registered until the application adopts
    // it, so deliver the indication through the listening association's callback
    auto effectiveCallback = callback;
    if (effectiveCallback == nullptr && isToBeAccepted()) {
        if (auto listeningAssoc = sctpMain->getAssoc(listeningAssocId))
            effectiveCallback = listeningAssoc->callback;
    }
    if (effectiveCallback == nullptr)
        throw cRuntimeError("No callback registered for SCTP socket, socketId = %d", assocId);
    effectiveCallback->handleEstablished(msg);
}
...
void SctpAssociation::sendAbort(uint16_t tBit)
{
    SctpAuthenticationChunk *authChunk;
...
static uint32_t copyToRGaps(SctpSackChunk *sackChunk,
        const SctpGapList *gapList,
        const SctpGapList::GapType type,
        const bool compression,
        size_t& space)
{
    const uint32_t count = gapList->getNumGaps(type);
    uint32_t last = gapList->getCumAckTsn();
    uint32_t keys = min((uint32_t)(space / 4), count); // Each entry occupies 2+2 bytes => at most space/4 entries
    if (compression) {
...
static uint32_t copyToNRGaps(SctpSackChunk *sackChunk,
        const SctpGapList *gapList,
        const SctpGapList::GapType type,
        const bool compression,
        size_t& space)
...
void SctpAssociation::sendDataArrivedNotification(uint16_t sid)
{
...
void SctpAssociation::sendHMacError(const uint16_t id)
{
    const auto& sctpmsg = makeShared<SctpHeader>();
    sctpmsg->setChunkLength(B(SCTP_COMMON_HEADER));
    SctpErrorChunk *errorChunk = new SctpErrorChunk();
...
void SctpAssociation::putInDeliveryQ(uint16_t sid)
{
    ...
    do {
               ...
               (!restrict || (restrict && state->pushMessagesLeft > 0)))
        {
          ...
          /*  bool dummy;
            ...
            if (assocStat.numEndToEndMessages >= assocStat.startEndToEndDelay &&
                (assocStat.numEndToEndMessages < assocStat.stopEndToEndDelay || !assocStat.stopEndToEndDelay))
            {
                assocStat.cumEndToEndDelay += (simTime() - chunk->firstSendTime);
            }
            assocStat.numEndToEndMessages++;
...
bool SctpAssociation::makeRoomForTsn(const uint32_t tsn, const uint32_t length, const bool uBit)
{
...
bool SctpAssociation::tsnIsDuplicate(const uint32_t tsn) const
{
...
bool SctpAssociation::nextChunkFitsIntoPacket(SctpPathVariables *path, int32_t bytes)
{
    int32_t nextStream = -1;
...
void SctpAssociation::putInTransmissionQ(const uint32_t tsn, SctpDataVariables *chunk)
{
