...
bool TcpReno::packetDelayed() const
{
...
{
    // Linux tcp_may_undo(): undo when every retransmission of the episode has
    // been D-SACKed (undoRetrans == 0), or when the Eifel timestamp test proves
    // the ACKs were generated by the original transmissions.
...
void TcpReno::processRexmitTimer(TcpEventCode& event)
{
    ...
    // Open the RTO episode's undo context BEFORE the ssthresh/cwnd reduction
    // (Linux tcp_enter_loss -> tcp_init_undo; RFC 5682 sec 3.2: recurring
    // timeouts on the same SND.UNA keep the ORIGINAL context, hence the
    // undoMarker guard). Both spuriousness proofs restore from it: F-RTO's
    // never-retransmitted-data-SACKed test and Eifel's timestamp test.
    if ((state->lossUndoEnabled || (state->frtoEnabled && state->sack_enabled))
            && state->undoMarker == 0)
        undoInit();

    // F-RTO (RFC 5682, SACK-enhanced): remember snd_max ("recover") so the
    // episode can be closed once everything outstanding at the RTO is
    // accounted for.
...
void TcpReno::receivedDataAck(uint32_t firstSeqAcked)
{
    ...
    // Eifel evidence is one-shot (Linux clears retrans_stamp once retransmitted
    // data is acked): only the FIRST snd_una-advancing ACK after a
    // retransmission can testify about it -- later ACKs echo timestamps that
    // postdate the retransmission in normal operation, and letting a stale
    // stamp linger made every late ACK of a reordering-heavy transfer look
    // like proof of spuriousness (undo fired dozens of times per connection).
    bool undoNow = state->lossUndoEnabled && mayUndo();
    state->retransStampTS = 0; // consumed by the evaluation above, one way or the other

    if (undoNow) {
        // RFC 2883/3522: every retransmission of this episode has been D-SACKed, or
        // the Eifel timestamp proves the ACKs came from the originals -- the
