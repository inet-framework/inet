...
void TcpReno::receivedDuplicateAck()
{
    ...
    if (state->dupacks == state->dupthresh) {
        ...
        // Resetting the REXMIT timer is discussed in RFC 2582/6582 (NewReno) and RFC 6298.
        ...
        if (state->sack_enabled) {
            ...
            check_and_cast<Rfc6675Recovery *>(recovery)->setPipe();
            // RFC 6675, page 9: "(4.5) In order to take advantage of potential additional available
            // cwnd, proceed to step (C) below."
            if (state->lossRecovery) {
                // RFC 6675, page 11: "Therefore, we give implementers the latitude to use the standard
        ...
        // Note: Steps (A) - (C) of RFC 6675, page 9 ("Once a TCP is in the loss recovery phase, the following procedure MUST be used for each arriving ACK")
        // should not be used here!

        // RFC 6675, page 9: "5.1 Retransmission Timeouts
        // (...)
        // If there are segments missing from the receiver's buffer following
        // processing of the retransmitted segment, the corresponding ACK will
        // contain SACK information.  In this case, a TCP sender SHOULD use this
