...
void Tcp::processIcmpv4Error(Indication *indication)
{
    ...
    EV_WARN << "ICMPv4 error received: type=" << errorInd->getType()
            << " code=" << errorInd->getCode()
...
void Tcp::processIcmpv6Error(Indication *indication)
{
    ...
    EV_WARN << "ICMPv6 error received: type=" << errorInd->getType()
            << " code=" << errorInd->getCode()
            << " about packet " << localAddr << ":" << localPort << " > "
            << remoteAddr << ":" << remotePort << "\n";
...
std::vector<uint8_t> Tcp::generateFastOpenCookie(const L3Address& localAddr, const L3Address& remoteAddr, int cookieBytes)
{
    ...
    // written out little-endian. Verified against the kernel packetdrill
    // corpus's published key->cookie vectors (fastopen/server/simple1-3).
