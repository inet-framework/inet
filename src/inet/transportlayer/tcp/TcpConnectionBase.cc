...
TcpStateVariables::active(false), fork(false), snd_mss(0), snd_una(0), snd_nxt(0), snd_max(0), snd_wnd(0), snd_up(0), snd_wl1(0), snd_wl2(0), iss(0), rcv_nxt(0), rcv_wnd(0), rcv_up(0), irs(0), rcv_adv(0), syn_rexmit_count(0), syn_rexmit_timeout(0), fin_ack_rcvd(false), send_fin(false), snd_fin_seq(0), fin_rcvd(false), rcv_fin_seq(0), nagle_enabled(false), delayed_acks_enabled(false), limited_transmit_enabled(false), increased_IW_enabled(false), full_sized_segment_counter(0), ack_now(false), afterRto(false), time_last_data_sent(0), ws_support(false), ws_enabled(false), ws_manual_scale(-1), snd_ws(false), rcv_ws(false), rcv_wnd_scale(0), snd_wnd_scale(0), ts_support(false), ts_enabled(false), snd_initial_ts(false), rcv_initial_ts(false), ts_recent(0), last_ack_sent(0), sack_support(false), sack_enabled(false), snd_sack_perm(false), rcv_sack_perm(false), snd_sack(false), snd_dsack(false), start_seqno(0), end_seqno(0), highRxt(0), pipe(0), recoveryPoint(0), sackedBytes(0), sackedBytes_old(0), lossRecovery(false), dupacks(0), snd_sacks(0), rcv_sacks(0), rcv_oooseg(0), rcv_naseg(0), maxRcvBuffer(0), usedRcvBuffer(0), freeRcvBuffer(0), tcpRcvQueueDrops(0), sendQueueLimit(0), queueUpdate(true), sndCwr(false), ecnEchoState(false), gotEce(false), gotCeIndication(false), ect(false), endPointIsWillingECN(false), ecnSynSent(false), ecnWillingness(false), sndAck(false), rexmit(false), TcpStateVariables, eceReactionTime(0)()
{
    // set everything to 0 -- real init values will be set manually
    
    
       // will initially be set from configureStateVariables() and probably reset during connection setup
       ...
       // will be set from configureStateVariables()
