...
#ifdef INET_WITH_IPv4
        {
            auto icmpGate = findModuleInterface(gate("ipOut"), typeid(Icmp), nullptr);
            if (icmpGate)
...
#ifdef INET_WITH_IPv6
        {
            auto icmpGate = findModuleInterface(gate("ipOut"), typeid(Icmpv6), nullptr);
            if (icmpGate)
...
void Udp::processUndeliverablePacket(Packet *udpPacket)
{
    ...
    if (protocol->getId() == Protocol::ipv4.getId()) {
        if (icmpv4Module == nullptr)
            throw cRuntimeError("Cannot send ICMP error message, no ICMP module found from gate ipOut");
        ...
        if (icmpv6Module == nullptr)
            throw cRuntimeError("Cannot send ICMPv6 error message, no ICMPv6 module found from gate ipOut");
        udpPacket->setFrontOffset(udpPacket->getTag<NetworkProtocolInd>()->getNetworkHeaderFrontOffset());
...
void Udp::sendUp(Ptr<const UdpHeader>& header, Packet *payload, SockDesc *sd, ushort srcPort, ushort destPort)
{
    ...
    deferrablePushPacket(appSink, payload);
    numPassedUp++;
}
