...
class OppTestTask(TestTask):
    ...
    def run_protected(self, **kwargs):
        ...
        if subprocess_result.returncode == -signal.SIGINT.value:
            ...
            return self.task_result_class(self, result="FAIL", reason="Non-zero exit code")
...
def get_opp_test_tasks(test_folder, simulation_project=default_project, filter=".*", full_match=False, **kwargs):
    def create_test_task(test_file_name):
        return OppTestTask(simulation_project.get_full_path(test_folder), os.path.basename(test_file_name), task_result_class=TestTaskResult, **kwargs)
