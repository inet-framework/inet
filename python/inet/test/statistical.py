...
class StatisticalTestTask(SimulationTestTask):
    ...
    def run_protected(self, **kwargs):
        self.simulation_task.remove_result_folder()
    ...
    def check_simulation_task_result(self, simulation_task_result, result_name_filter=None, exclude_result_name_filter=None, result_module_filter=None, exclude_result_module_filter=None, full_match=False, **kwargs):
        ...
        current_results_directory = simulation_project.get_full_path(os.path.join(working_directory, self.simulation_task.result_folder))
...
def get_statistical_test_tasks(sim_time_limit=get_statistical_test_sim_time_limit, run_number=0, **kwargs):
    ...
    """
    multiple_simulation_test_tasks = get_simulation_tasks(name="statistical test", run_number=run_number, sim_time_limit=sim_time_limit, simulation_task_class=StatisticalTestTask, multiple_simulation_tasks_class=MultipleSimulationTestTasks, **kwargs)
    for simulation_test_task in multiple_simulation_test_tasks.tasks:
        simulation_task = simulation_test_task.simulation_task
        simulation_task.result_folder = "results." + simulation_task.get_working_directory_relative_unique_result_folder()
    return multiple_simulation_test_tasks
...
def run_statistical_tests(**kwargs):
    ...
    return multiple_statistical_test_tasks.run(extra_args=["--**.param-recording=false", "--**.vector-recording=false"], **kwargs)
...
class StatisticalResultsUpdateTask(SimulationUpdateTask):
    ...
    def run_protected(self, **kwargs):
        ...
        working_directory = self.simulation_task.simulation_config.working_directory
        ...
        if update_result.result == "UPDATE":
            for scalar_result_file_name in glob.glob(os.path.join(self.simulation_task.get_result_folder_full_path(), "*.sca")):
...
def get_update_statistical_result_tasks(run_number=0, **kwargs):
    ...
    Returns (:py:class:`MultipleUpdateTasks`):
        ...
        The result can be run (and re-run) without providing additional parameters.
    """
    multiple_simulation_update_tasks = get_simulation_tasks(run_number=run_number, multiple_simulation_tasks_class=MultipleSimulationUpdateTasks, simulation_task_class=StatisticalResultsUpdateTask, **kwargs)
    for simulation_update_task in multiple_simulation_update_tasks.tasks:
        simulation_task = simulation_update_task.simulation_task
        simulation_task.result_folder = "results." + simulation_task.get_working_directory_relative_unique_result_folder()
    return multiple_simulation_update_tasks
...
def update_statistical_results(sim_time_limit=get_statistical_test_sim_time_limit, **kwargs):
    ...
    return multiple_update_statistical_result_tasks.run(sim_time_limit=sim_time_limit, extra_args=["--**.param-recording=false", "--**.vector-recording=false"], **kwargs)
