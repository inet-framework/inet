...
class FingerprintTestTask(SimulationTestTask):
    ...
    def run(self, test_result_filter=None, exclude_test_result_filter="SKIP", output_stream=sys.stdout, append_args=[], **kwargs):
        if self.fingerprint and matches_filter(self.test_result, test_result_filter, exclude_test_result_filter, True):
            ...
            return super().run(append_args=append_args + self.get_append_args(simulation_project, str(self.fingerprint)) + get_ingredients_append_args(self.ingredients), output_stream=output_stream, **kwargs)
        else:
...
class FingerprintTrajectoryDivergencePosition:
    ...
    def __repr__(self):
        ...
        return "Fingerprint trajectory divergence at " + \
               COLOR_CYAN + simulation_project_1.get_name() + COLOR_RESET + " #" + COLOR_GREEN + str(self.simulation_event_1.event_number) + COLOR_RESET + ", " + \
               COLOR_CYAN + simulation_project_2.get_name() + COLOR_RESET + " #" + COLOR_GREEN + str(self.simulation_event_2.event_number) + COLOR_RESET
