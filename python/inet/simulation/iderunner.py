    ...
    def run(self, simulation_task, args):
        ...
        if simulation_task.debug:
            if simulation_task.break_at_matching_event:
                debugger_init_commands = [f"breakpoint set -G true -n cSimulation::setupNetwork -C \"expression (void) omnetpp::cSimulation::getActiveSimulation()->setDebuggerBreakConditionFunction((bool(*)(omnetpp::cEvent*))[] (omnetpp::cEvent *event) -> bool {{ return {simulation_task.break_at_matching_event.replace('\"', '\\\"')}; }})\""]
            elif simulation_task.break_at_event_number:
                debugger_init_commands = [f"breakpoint set -G true -n cSimulation::setupNetwork -C \"expression (void) omnetpp::cSimulation::getActiveSimulation()->setDebuggerBreakConditionFunction((bool(*)(omnetpp::cEvent*))[] (omnetpp::cEvent *event) -> bool {{ return omnetpp::cSimulation::getActiveSimulation()->getEventNumber() == {simulation_task.break_at_event_number - 1}; }})\""]
            else:
