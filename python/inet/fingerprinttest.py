...
def fpGroup2Str(fpGroup, separ1=';', separ2=' '):
    return separ1.join(separ2.join(sorted(x)) for x in fpGroup)


def str2FpGroup(str, separ1=';', separ2=' '):
    return list(list(x.split(separ2)) for x in str.split(separ1))


def filteringFpGroup(fpGroup, fpFilterList, excludeFpFilterList):
    if len(fpFilterList):
        # filtering with fpFilterList
        l1 = list()
        for i in range(len(fpGroup)):
            s = list(filter(lambda x : x.split('/', 1)[1] in fpFilterList, fpGroup[i]))
            if s:
                l1.append(s)
    else:
        l1 = fpGroup

    if len(excludeFpFilterList):
        # filtering with excludeFpFilterList
        l2 = list()
        for i in range(len(l1)):
            s = list(filter(lambda x : x.split('/', 1)[1] not in excludeFpFilterList, l1[i]))
            if s:
                l2.append(s)
    else:
        l2 = l1
...
def fpGroupMerge(mFp, otherFp):
    for i in range(len(otherFp)):
        if i >= len(mFp):
            mFp.append(list())
        s = list(filter(lambda x : x not in mFp[i], otherFp[i]))
        if s:
            mFp[i].extend(s)
        #mFp[i].append(otherFp[i])  # TODO filter to unique


def diffFpGroups(actualFpGroup, expectedFpGroup):
        ...
        for simulation in simulations:
            ...
            if not filterRegexList or ['x' for regex in filterRegexList if re.search(regex, title)]: # if any regex matches title
                if not excludeFilterRegexList or not ['x' for regex in excludeFilterRegexList if re.search(regex, title)]: # if NO exclude-regex matches title
                    fingerprints = str2FpGroup(simulation['fingerprint'])
                    if len(fpFilterList) > 0 or len(excludeFpFilterList) > 0:
                        fingerprints = filteringFpGroup(fingerprints, fpFilterList, excludeFpFilterList)
                    if len(fingerprints):
                        fpArg = set()
                        fpSet = set()
        ...
        for simulation in simulations:
            if 'computedFingerprint' in simulation:
                ...
                if len(oldFingerprint) == len(newFingerprint):
                    ...
                    print("ERROR: Cannot replace fingerprint '%s' to '%s' at '%s' line %d:\n     %s" % (fpGroup2Str(oldFingerprint), fpGroup2Str(newFingerprint), csvFile, line, lines[line]))
...
class SimulationTestCase(unittest.TestCase):
    def runSimulation(self, title, command, workingdir, resultdir):
        ...
        fps = fp + '(?: '+fp+')*'
        fpsl = fps + '(?:, '+fps+')*'
        pattern = 'calculated: (' + fpsl + '), expected: (' + fpsl + ')$'
            ...
            if re.search("Fingerprint", err):
                if re.search("Fingerprint successfully", err):
                    ...
                    if m:
                        ...
                        result.computedFingerprint = str2FpGroup(m.group(1), ', ')
                        result.expectedFingerprint = str2FpGroup(m.group(2), ', ')
                    else:
...
class FingerprintTestCase(SimulationTestCase):
    ...
    def runTest(self):
        ...
        command = (exeName + "_dbg" if debug else exeName + "_release" if release else exeName) + " -u Cmdenv " + progArgs + ' ' + self.extraFpArg + \
            ...
            " \"--fingerprint=" + fpGroup2Str(self.fingerprint, ', ') + "\" --cpu-time-limit=" + cpuTimeLimit + \
        ...
        for rep in range(self.repeat):
            ...
            if len(curFp):
                fpGroupMerge(computedFingerprints, curFp)
        ...
        if anyFingerprintBad:
            ...
            assert False, "some fingerprint mismatch: actual '" + fpGroup2Str(afp) + "', expected: '" + fpGroup2Str(efp) + "'"
...
def run_fingerprint_tests(testspecfiles = None, calculator =  defaultFingerprintCalculator, fpfilter = [], excludefpfilter = [], match = [], exclude = [], threads = 1, repeat = 1, n = 1, i = 0):
    global executable, rootDir, extraOppRunArgs, logFile
    executable = "inet"
    rootDir = os.path.abspath(".")  # the working directory in the CSV file is relative to this dir
    if calculator:
        extraOppRunArgs += " --fingerprintcalculator-class=" + calculator
    od = os.getcwd()
    try:
        os.chdir("tests/fingerprint")
        if not testspecfiles:
            testspecfiles = glob.glob('*.csv')
        generator = FingerprintTestCaseGenerator()
        testcases = generator.generateFromCSV(testspecfiles, fpfilter, excludefpfilter, match, exclude, repeat)
        testcases = testcases[i::n]
        testSuite = ThreadedTestSuite()
        testSuite.addTests(testcases)
        testSuite.thread_count = threads
        testSuite.repeat = repeat
