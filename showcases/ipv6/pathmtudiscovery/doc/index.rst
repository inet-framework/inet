...
In INET the filtering can be done with the Security Policy Database, which is a packet
...
a policy is enough. These two lines belong to the ``BlackHole`` configuration only; in
the other three the firewall forwards everything:
