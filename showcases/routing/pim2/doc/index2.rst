Multicast Routing with PIM (Redesigned)
========================================
...
IP multicast solves this by letting the *network* replicate packets: the
source sends each packet once, and routers along the path duplicate it only
where the distribution tree branches. Each link carries only one copy,
regardless of how many receivers are downstream. Protocol Independent
Multicast (PIM) is the protocol that builds and maintains these distribution
trees.
...
.. admonition:: In one minute
   ...
   - With plain unicast, a server streaming to N receivers sends N identical
     copies; with IP multicast, routers replicate the packets, so every link
     carries the stream only once.
   - PIM builds the multicast distribution trees. It is "protocol
     independent": it has no topology discovery of its own and relies on the
     unicast routing table.
   - **Dense Mode** floods traffic everywhere, then prunes branches with no
     receivers. **Sparse Mode** forwards nothing until receivers explicitly
     join, meeting sources at a *Rendezvous Point*.
   - Demonstrated here: PIM-DM flood/prune/graft and PIM-SM's
     Register-to-native-path switchover on a small tree network, plus an IPTV
     stream over a 3×3 router grid.
   - Page roadmap: PIM background → PIM in INET (modules, configuration) →
     three scenarios, each with videos.

The showcase's finale in one shot — the IPTV scenario, where a PIM-SM
multicast tree forms across a 3×3 router grid as soon as the client joins:

.. video:: media/Iptv_tree_building.mp4

**When the client joins, PIM builds the multicast tree on demand: the IPTV
stream lights up a single path across the 3×3 router grid, and each link
carries only one copy of the stream.**

About PIM
---------

On the local network segment, the last-hop router sends each packet as a
multicast frame — all interested hosts receive it directly. Hosts signal their
interest to their local router using IGMP (Internet Group Management
Protocol): a host sends an IGMP Report to join a multicast group, and the
router periodically sends IGMP Queries to check whether any group members are
still present on the link.

PIM is called "protocol independent" because it does not include its own
topology discovery mechanism — instead it relies on the unicast routing table
...
PIM operates in two primary modes, each suited to a different multicast
traffic pattern.
...
.. admonition:: Details — State Refresh: suppressing the periodic re-flood

   To avoid the periodic flood-and-prune cycle, the source's first-hop router
   (also called the *designated router* or DR) can periodically send **State
   Refresh** messages (every 60s by default) downstream to maintain prune
   state without re-flooding.
...
sources don't know where the receivers are.

The protocol solves this by designating one router as the *Rendezvous Point
(RP)* — a well-known meeting point. Receivers send Join messages *towards* the
RP, and sources send their traffic *to* the RP. The RP connects the two sides,
and the tree of paths that forms around it is called the *shared tree*.

All routers in the PIM domain must know the RP's address — in real networks
this is typically learned via the PIM Bootstrap mechanism, but in INET it is
configured statically on every router.
...
an ``(S,G)`` Join asks for an ``(S,G)`` forwarding entry to be created.
Receivers typically send ``(*,G)`` Joins because they just want the stream
without caring who sends it, while the RP uses ``(S,G)`` Joins when it needs
to pull traffic from a particular source.
...
- When a source starts sending, it simply sends a normal multicast IP packet
  ...
  wraps it inside a unicast **Register** message addressed to the RP. This
  *registers* the source with the RP — informing it that a new source is
  active for this group, while simultaneously delivering the data. Register
  encapsulation is a temporary bootstrap mechanism: it gets traffic flowing
  immediately, and the protocol replaces it with native multicast as quickly
  as possible.
...
.. admonition:: Details — Register overhead and Register-Null probing

   Register encapsulation adds overhead: each packet carries an extra header,
   and the RP must individually decapsulate every packet from every source.
   With many simultaneous sources this can turn the RP into a bottleneck —
   which is why the protocol transitions to native multicast as quickly as
   possible.

   After receiving Register-Stop, the DR starts a *Register-Stop Timer*. When
   the timer nears expiry, the DR sends a **Register-Null** message (a
   Register with no data) to the RP to check whether receivers still exist.
   If they do, the RP replies with another Register-Stop and the cycle
   repeats. This probing keeps the source's DR in sync with the RP without
   sending actual data.
...
.. admonition:: Fine print — INET's simplified Register-Stop timing
...
The :ned:`MulticastRouter` node type is a standard :ned:`Router` with PIM and
IGMP protocols enabled (``hasPim = true``) and multicast forwarding turned on
by default. Every router along a multicast path must run PIM so it can process
Join/Prune messages and build its own multicast forwarding state.

Inside each router, the compound module ``pim`` contains a :ned:`PimDm` and a
:ned:`PimSm` submodule (both always present), along with a splitter that
directs incoming PIM packets to the appropriate mode based on the receiving
interface's configuration.

.. figure:: media/PimModule.png

**Both PIM modes are always present in every router — the splitter in front
of them dispatches each incoming PIM packet according to the receiving
interface's configured mode.**

PIM relies on *Reverse Path Forwarding (RPF) checks* to prevent loops. When a
multicast packet arrives at a router, PIM looks up the source address (or the
RP address, for shared trees) in the unicast routing table and checks whether
the packet arrived on the interface that leads back towards that address.

If it did, the packet is accepted and forwarded. If it arrived on a different
interface — for example, because of a redundant path in the network — the
packet is dropped. This prevents forwarding loops and duplicate delivery. The
...
**Configuring the Rendezvous Point (PIM-SM only).** The RP address is set via
the ``RP`` parameter of the :ned:`PimSm` module (path: ``**.pim.pimSM.RP``).
All routers are configured with the same address. The router that owns this
address recognizes itself as the RP.

You can assign a well-known address to any interface on the RP router using
the :ned:`Ipv4NetworkConfigurator` XML — the configurator will then
automatically generate unicast routes to it from all other routers (which is
necessary for Join and Register messages to reach the RP):
...
.. admonition:: Fine print — INET PIM-SM limitations vs. RFC 4601
...
We use two network topologies in this showcase. The first,
``PimShowcaseNetwork``, is a simple tree topology used for both the PIM-DM and
PIM-SM configurations. A :ned:`StandardHost` (``source``) sends UDP multicast
traffic to the group address ``239.1.1.1`` through five :ned:`MulticastRouter`
nodes (R0–R4) to three receivers.

The receivers run :ned:`UdpSink` applications configured to join this
multicast group.

.. figure:: media/PimShowcaseNetwork.png

**R0 is the source's directly connected router, R1 is the tree root, and R2,
R3, and R4 branch towards the three receivers.**
...
(:ned:`StandardHost`) attached at one corner and a client at the opposite
corner.

.. figure:: media/PimIptvNetwork.png

**The 3×3 grid offers multiple possible paths between the server's corner and
the client's corner — PIM picks one using the unicast routing tables.**
...
**Flooding delivers the stream to every router at first; R3's Prune then cuts
off the branch that has no receivers, leaving only receiver1's path active.**
...
**The Graft message reattaches R3's pruned branch on the spot — receiver2
starts receiving immediately, without waiting for the next flood cycle.**
...
**Register encapsulation (red) bootstraps delivery to the RP; once the (S,G)
Join establishes the native path, the stream flows as plain multicast (blue)
with no unicast wrapper.**
...
- At 25s, the IPTV server starts sending high-rate multicast traffic (one
  1000-byte packet every 40ms, i.e. 25 packets/s).
- At 30s, the client joins the multicast group. R22 (the client's designated
  router) sends a (*,G) Join towards the RP (R10).
- The join propagates through the grid, and the multicast tree is established
  along the shortest path from the server's corner (R00) through the RP (R10)
  to the client's corner (R22). Once the tree is built, the client begins
...
**The stream follows a single shortest path — R00 through the RP (R10) to
R22 — while the rest of the grid stays idle: sparse mode forwards only along
branches that have been explicitly requested.**
