...
sent packets is verified by the python script (``compute_frame_replication_success_rate_analytically2()`` function in ``inet/python/tests/validation.py``). The expected result is around 0.657.
...
.. code-block:: bash
    ...
    $ opp_env run inet-4.4 --init -w inet-workspace --install --chdir \
       -c 'cd inet-4.4.*/showcases/tsn/framereplication/automaticfailureprotection && inet'
