...
The Single Rate Two Color Meter (srTCM) is a specific implementation of the token bucket
algorithm defined in RFC 2697. In INET, this is implemented by the :ned:`SingleRateTwoColorMeter` module:

- **Single token bucket**: Uses one bucket with Committed Information Rate (CIR) and Committed Burst Size (CBS) parameters
- **Two-color marking**: Packets are labeled as either "green" (conforming) or "red" (non-conforming)
- **Forwarding decision**: Green packets are allowed through, while red packets are typically dropped by subsequent filters

This metering approach is simpler than three-color alternatives but provides effective rate limiting
with burst tolerance for TSN applications.
