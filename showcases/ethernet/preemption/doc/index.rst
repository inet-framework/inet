...
Ethernet preemption (802.1Qbu) enables higher priority frames to interrupt the transmission 
...
| INET version: ``4.3``
| Source files location: `inet/showcases/ethernet/preemption <https://github.com/inet-framework/inet-showcases/tree/master/ethernet/preemption>`__
...
- ``Default``: The baseline configuration; doesn't use priority queue or preemption.
...
The ``Default`` configuration uses no preemption or priority queue, the configuration just limits 
...
In the ``PriorityQueue`` configuration, we change the queue type in the Mac layer from the 
...
In the ``Preemption`` configuration, we replace the :ned:`EthernetMacLayer` and 
...
We use the following traffic for the ``RealisticDefault``, ``RealisticPriorityQueue`` 
and ``RealisticPreemption`` configurations:
...
For the ``Default`` configuration, the MAC stores both background and high-priority 
...
For the ``PriorityQueue`` configuration, high-priority frames have their own sub-queue in 
...
For the ``Preemption`` configuration, the high-priority frames have their own queue in the MAC.
...
Sources: :download:`omnetpp.ini <../omnetpp.ini>`, :download:`EthernetPreemptionShowcase.ned <../EthernetPreemptionShowcase.ned>`
