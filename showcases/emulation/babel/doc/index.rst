...
We can use the :ned:`ExternalEnvironment` and :ned:`ExternalProcess` modules to specify commands to run on the host OS from the ini file.

V1 :ned:`ExternalEnvironment` can execute commands on the host OS at module creation and module destruction.
...
shared parts of the command by parameterizing network-node-dependent values such as node index. **TODO** self-contained

So

- ExternalEnvironment can execute commands in the host OS to set up environment pertaining to that host, such as network namespaces.
It has parameters that specify commands to execute at module creation and destruction.
- ExternalProcess is similar, but it executes a command at runtime. We use it to start the babeld instances

Basically, we can get an ExternalProcess by inserting an ExternalApp. This has an ExternalProcess and ExternalEnvironment module

- app's ExternalEnvironment vs hosts'?
...
``inet/showcases/emulation/babel`` directory. From there, you can execute the
