...
signals between them are simulated. The ping requests are generated by a command-line ping
program, the ping replies by the real ICMP protocol,
...
We need to set up the network namespaces and TAP devices in the 'real' part of
the setup. This is summarized here; more details are in the Running section below.
...
TAP device, and an IPv4 routing table. 
We will assign the following IP addresses when creating the TAP devices:
...
We use a command-line ping program in ``host0``
to ping ``host2``. The ping response is generated by the ICMP protocol in the
host OS network stack.

In the 'simulated' part of the setup, we configure the Ext interfaces in :download:`omnetpp.ini <../omnetpp.ini>`:
...
.. As mentioned above, two ways are provided for running the emulation scenario.

.. We provide two ways to set up network namespaces, and run the babel daemons and INET:

We provide two ways for running the emulation scenario. The first one is using shell scripts to set up network namespaces, start the babel daemons and run INET.
The second one makes use of Mininet, a Software Defined Networking tool. These are detailed in the next sub-sections.

.. - Using shell scripts to set up network namespaces, start the babel daemons and run INET
.. - 

.. - Shell scripts: We set up and run everything manually
.. - Mininet: The network namespaces is built in Mininet, a tool for Rapid Prototyping of Networks.

.. a more traditional method with shell scripts
.. using mininet, 
...
Alternatively, the emulation scenario can be run in `Mininet <https://mininet.org>`__, an emulator for rapid prototyping of Software Defined Networks.
With Mininet, one can create virtual network topologies with just a few shell commands, or using Mininet's Python API.
Mininet typically takes the place of the 'real' part of the network in emulation scenarios, making it more convenient to build
networks. In this scenario, however, the real part of the network is just applications running on virtual Mininet hosts; the network
itself is simulated in INET. Nevertheless, using Mininet can be beneficial due to the convenience provided by its Python API.

.. Nevertheless, using Mininet can make it easier to integrate projects into others, TODO or use Python / you want to do something more complex in Python

To run the scenario with Mininet, use the ``start_mininet.sh`` script:

.. code-block:: bash

  ./start_mininet.sh

This wrapper script starts the :download:`mininet-tap.py <../mininet-tap.py>`, which creates a Mininet network with virtual hosts.
The python script starts the simulation, and runs the babel daemons on these hosts:

.. literalinclude:: ../mininet-tap.py
   :language: python

.. note:: Check out the :doc:`/showcases/emulation/voip/doc/index` showcase for more information about Mininet integration.
