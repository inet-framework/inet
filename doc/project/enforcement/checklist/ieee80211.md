...
> **Kind:** procedure · **Status:** current · **Seal:** none · **Owns:** — · **Stands on:** [domain/ieee80211.md](../../domain/ieee80211.md), [general.md](general.md), [review-a-code-change.md](../../guide/review-a-code-change.md)
...
Use the general checklist's exact definitions for `PASS`, `N/A`, `FLAG`, and `QUESTION`, and all of
its ground rules. In particular, `FLAG` requires a clear violation of the canonical rule cited by
the item; `QUESTION` requires a plausible rule conflict and genuine T5 judgment and carries no
defect severity. Missing investigation or runtime evidence is normally `not verified` or a residual
risk, not automatically a `QUESTION`. When a correctness finding already explains the same rule
violation, make the checklist `FLAG` reference that finding instead of duplicating it.
...
End with the same one-line verdict as the general checklist:
`REVIEW: n PASS, n N/A, n FLAG, n QUESTION`. List any `not verified` checks and residual risks
separately. For every `FLAG`, apply the general checklist's ledger-disposition rules.
