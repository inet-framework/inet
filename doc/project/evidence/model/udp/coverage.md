...
| [RFC1122-UCK-1](../../standard/rfc1122/catalog.md#rfc1122-uck-1) | covered | [checksum-by-default](../../protocol/udp/checks/checksum.md#checksum-by-default) | Rfc1122ChecksumDefault.test | the generate half **FAILS** by default, gap 1, declared expected because the repair is blocked; the check half PASSES, in four tests that set the computed mode |
| [RFC1122-UCK-2](../../standard/rfc1122/catalog.md#rfc1122-uck-2) | no check | — | — | a permission for a program interface |
| [RFC1122-UCK-3](../../standard/rfc1122/catalog.md#rfc1122-uck-3) | selected | [checksum-by-default](../../protocol/udp/checks/checksum.md#checksum-by-default) | Rfc1122ChecksumDefault.test | **FAIL (expected)**, gap 1, the repair is blocked |
...
| [RFC1122-UADDR-1](../../standard/rfc1122/catalog.md#rfc1122-uaddr-1) | selected | [multicast-source-address](../../protocol/udp/checks/input-validation.md#multicast-source-address) | Rfc1122MulticastSourceAddress.test | **FAIL (expected)**, gap 2, unimplemented |
...
27 entries. 17 reached a test: 15 with a PASS and 2 with a FAIL -- 1 of them a defect with a
blocked repair and 1 an unimplemented behavior -- that names a model gap. 9 carry `no check`,
each with its reason, and 1 waits for a unit-test suite. The nine
...
| [UDP-F-HEADER](../../protocol/udp/features.md#udp-f-header) | RFC768-HDR-3 PASS, with data and at the minimum, since `d63f1fdef5` | **supported** |
...
of level 2 supported. `UDP-F-CHECKSUM` is partial now: RFC 1122 entered the in-scope set, and
it demands of a host what RFC 768 only describes. `UDP-F-HEADER` also went partial, for a
different reason: a defect in a result filter, not in UDP, stopped the run at the minimum
length. `d63f1fdef5` repaired it on 2026-09-15, and the feature is supported again.
...
2. `UDP-F-HEADER` was supported for datagrams that carry data, and not yet for the minimum
   of eight; gap 3. Commit `d63f1fdef5` fixed the result filter that stopped the run on an
   empty payload, on 2026-09-15, and the feature is supported again.
...
1. The minimum length of eight: **reached** at level 3. It was gap 3 until `d63f1fdef5`
   repaired it on 2026-09-15; the minimum passes now.
...
| 3, Edge | **partial** | All 18 statements of RFC 1122 §4.1 are in the catalog; 9 have a check that ran, and the 9 at the program interface have none. Three model gaps found; `d63f1fdef5` repaired one, gap 3, on 2026-09-15. |
...
| 3 | 2026-09-23 | **3, partial** | Re-run only, no new check. The run record of the last pass named a commit that the landing rebase removed; this row gives the ledger a run on master. | 13 tests: 11 PASS, 2 FAIL (expected), 0 FAIL (unexpected). Since the last pass: `Rfc768EmptyDatagram.test` FAIL (unexpected) → PASS, gap 3 repaired by `d63f1fdef5` (2026-09-15); `Rfc1122ChecksumDefault.test` FAIL (unexpected) → FAIL (expected), gap 1 declared a defect with a blocked repair by `4548adeb04` (2026-09-17); `UDP-F-HEADER` support partial → supported; see [`results.md`](results.md) and [`conformance.md`](conformance.md) |
