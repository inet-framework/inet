...
The [model architecture](../design/ieee80211-model-architecture.md) defines the state categories,
ownership, lifecycle, and notification contracts used to assess changes within these boundaries.
...
Before classifying behavior as missing or incorrect, establish four independent facts:

| Fact | Established by |
| --- | --- |
| The scenario requires the behavior | The scenario, test, or model contract |
| The applicable standard defines the behavior | The identified revision and clause under AR-WLAN-STD-TRACE |
| The checked-out INET version implements the behavior | Its NED contracts and source paths |
| This run enables the behavior | The effective configuration, local/peer capabilities, and runtime protocol state |

These facts are evaluated for the particular behavior and run; this rule does not maintain a mutable
inventory of implemented amendments. A standard definition does not prove implementation, the
presence of source code does not prove enablement, and a scenario that does not require a feature
does not fail because the feature is absent. Unsupported, implemented-but-disabled, not-required,
and incorrectly implemented behavior are different conclusions and require different changes.

Operating mode, local and peer capabilities, and prerequisite protocol state are explicit gates,
not an implicit "the code exists, so it runs." This lets multiple amendment generations share the
tree without destabilizing legacy paths: a legacy simulation produces the same trajectory when
newer code is merely present. The gate condition itself is normative surface and falls under
AR-WLAN-STD-TRACE.
...
A throughput change or success in an ideal/noiseless radio scenario demonstrates an outcome, not
the protocol mechanism that produced it. Mechanism claims are tested at the layer where the
required frame, state transition, gate, timing rule, retry, aggregation, or PHY decision is
observable. When the model intentionally replaces that mechanism with a declared abstraction, a
test may establish the abstraction-level contract, but the result must not be presented as evidence
that the omitted normative mechanism executed.
