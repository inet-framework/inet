...
| AV-CONTRACT-02 | **a body the Base could take today**: `IFunction::printOn()` → `FunctionBase` exists; `IIeee80211Band::printToStream()` → `Ieee80211BandBase` exists; `ITransmitStep/IReceiveStep::getType() { return TRANSMIT/RECEIVE; }` | `common/math/`, `physicallayer/wireless/ieee80211/mode/`, `linklayer/ieee80211/mac/contract/IFrameSequence.h` | Move the body down one level. Three small commits. | Open |
