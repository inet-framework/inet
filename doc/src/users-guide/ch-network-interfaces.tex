...
\begin{itemize}
    \item \textit{Layer 2 protocol implementation}. For some interfaces such as
      ...
      other subcomponents.
    \item \textit{PHY model}. Some interfaces also contain separate
      module(s) that implement the physical layer. For example,
      \nedtype{Ieee80211Interface} contains a radio module.
    \item \textit{Output queue}. This module is optional and absent by default,
    ...
    \item \textit{Traffic conditioners} allow traffic shaping and policing elements
      to be added to the interface, for example to implement a Diffserv router.
    \item \textit{Hooks} allow extra modules to be inserted in the incoming
