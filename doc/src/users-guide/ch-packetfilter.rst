...
with ``"ping"`` and that don't end with ``"reply"``:
...
Implicitly defined variables can be used to check the presence of a chunk of a
...
It is also possible to combine expressions using boolean operators:
