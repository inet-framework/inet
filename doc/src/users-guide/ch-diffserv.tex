...
Network interface modules, such as \nedtype{PppInterface} and
\nedtype{EthernetInterface}, may contain traffic conditioners in
their input and output data path. 

Network interfaces may also contain an optional external queue component. In the
absence of an external queue module, \nedtype{Ppp} and \nedtype{EtherMac} use an
internal drop-tail queue to buffer the packets while the line is busy.

\subsection{Traffic Conditioners}
\label{sec:diffserv:traffic-conditioners}

Traffic conditioners have one input and one output gate as defined in the
\nedtype{ITrafficConditioner} interface. They can transform the incoming traffic
by dropping or delaying packets. They can also set the DSCP field of the packet,
...
Queue components must implement the \nedtype{IOutputQueue} module interface. 
In addition to having one input and one output gate, these components
must implement a passive queue behaviour: they only deliver a packet when the
module connected to their output explicitly requests it.
(In C++ terms, the module must implement the \cppclass{IPassiveQueue} interface.
The next module requests a packet by calling the \ffunc{requestPacket()} method
of that interface.)
...
be forwarded to the \fgate{defaultOut} gate.
