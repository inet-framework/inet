...
\begin{itemize}
  ...
  \item \fpar{forceBroadcast} if \ttt{true}, then link-local broadcast
...
\begin{itemize}
  ...
  \item \fpar{multicastForwarding}: turns multicast IP forwarding on/off.
    Default is \ttt{false}, should be set to \ttt{true} in multicast routers.
\end{itemize}
...
\begin{itemize}
  ...
  \item \fpar{proxyARP}: enables proxy ARP mode (default is \ttt{true})
  \item \fpar{globalARP}: use global ARP cache (default is \ttt{false})
\end{itemize}
...
\begin{itemize}
  \item \fpar{enabled} if \ttt{false} then the IGMP module
