...
that can be combined in many ways. The primary means of building large
...
Assembling an INET simulation starts with defining a module that represents
the network. Networks are compound modules that contain network nodes,
automatic network configurators, and sometimes additionally a transmission
medium, physical environment, various visualizers, and other
...
-  :ned:`StandardHost` contains the most common Internet protocols:
   ...
   applications that are entirely configurable from INI files.
...
-  :ned:`WirelessHost` provides a network node with one (default)
   ...
   suitable for use with an :ned:`AccessPoint`.
...
Additionally, there are components that occur on the network level, but they
...
-  *Group coordinators* are needed for the operation of some group
   mobility models. For example, :ned:`MoBanCoordinator` is the
...
:protocol:`TCP` application that echos back all data received from the
...
due to the dynamically changing nature of connectivity. For wireless
...
This section contains quick and somewhat superficial advice on many practical tasks.
...
In many wired network simulations, the number of wired interfaces does
not need to be manually configured because it can be automatically inferred
...
both :protocol:`IPv4` and :protocol:`IPv6` are available. They are
