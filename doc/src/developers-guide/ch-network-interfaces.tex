...
\begin{itemize}
  \item \ttt{NF\_INTERFACE\_CREATED}: an interface entry has been
    created and added to the interface table
  \item \ttt{NF\_INTERFACE\_DELETED}: an interface entry is going
  ...
  \item \ttt{NF\_INTERFACE\_CONFIG\_CHANGED}: a configuration setting
    in an interface entry has changed (e.g. MTU or IP address)
  \item \ttt{NF\_INTERFACE\_STATE\_CHANGED}: a state variable in an
