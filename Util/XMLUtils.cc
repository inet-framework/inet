...
const cXMLElement* getUniqueChild(const cXMLElement *node, const char *name)
{
    ...
    if(!child)
        throw new cException("xml error: exactly one %s element expected", name);
...
const cXMLElement* getUniqueChildIfExists(const cXMLElement *node, const char *name)
{
    ...
    if(list.size() > 1)
        throw new cException("xml error: at most one %s element expected", name);
    else if(list.size() == 1)
        return (*list.begin());
    else
        return NULL;
...
bool parseBool(const char *text)
{
    if(!strcasecmp(text, "down"))
        ...
        throw new cException("unknown bool constant: %s", text);
}
