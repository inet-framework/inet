...
void send_packet(struct interface_FOO *ifp, 
		 struct stream *s,
		 u_int32_t dst,
		 struct prefix *p,
		 u_int32_t ttl);
...
irdp_sock_init (void)
{
  ...
  t_irdp_raw = thread_add_read (zebrad.master__item, irdp_read_raw, NULL, irdp_sock); 
...
int make_advertisement_packet (struct interface_FOO *ifp, 
			       struct prefix *p,
			       struct stream *s)
{
...
void irdp_send(struct interface_FOO *ifp, 
	       struct prefix *p, 
	       struct stream *s)
{
  struct zebra_if *zi=ifp->info;
...
void irdp_advertisement (struct interface_FOO *ifp, 
		   struct prefix *p)
{
  struct stream *s;
  s = stream_new (128);
...
{
  u_int32_t timer, tmp;
  struct interface_FOO *ifp = THREAD_ARG (t_advert);
  struct zebra_if *zi=ifp->info;
  struct irdp_interface *irdp=&zi->irdp;
  struct prefix *p;
  struct listnode *node;
  ...
  irdp->t_advertise = thread_add_timer(zebrad.master__item, irdp_send_thread, ifp, timer);
  return 0;
}
...
void process_solicit (struct interface_FOO *ifp)
{
  struct zebra_if *zi=ifp->info;
  struct irdp_interface *irdp=&zi->irdp;
  u_int32_t timer;
