...
void lifreq_set_name (struct lifreq *, struct interface_FOO *);
static int if_get_addr (struct interface_FOO *, struct sockaddr *);
static void interface_info_ioctl (struct interface_FOO *);
...
static int
if_get_addr (struct interface_FOO *ifp, struct sockaddr *addr)
{
  int ret;
  struct lifreq lifreq;
...
static void
interface_info_ioctl (struct interface_FOO *ifp)
{
  if_get_index (ifp);
  if_get_flags (ifp);
  if_get_mtu (ifp);
...
struct connected *
if_lookup_linklocal (struct interface_FOO *ifp)
{
#ifdef HAVE_IPV6
