...
static struct_bgp_master bgp_master_bgpd;

extern struct in_addr router_id_zebra_bgpd;

/* BGP process wide configuration pointer to export.  */
struct_bgp_master *bm_bgpd;

/* BGP community-list.  */
struct community_list_handler *bgp_clist_bgpd;

...
peer_group_get (struct bgp *bgp, const char *name)
{
  ...
  group->conf->host__item = strdup (name);
  group->conf->bgp = bgp;
...
bgp_create (as_t *as, const char *name)
{
  ...
  bgp->peer_self->host__item = strdup ("Static announcement");
...
struct peer_flag_action peer_flag_action_list_bgpd[9] = 
  {
...
bgp_master_init ()
{
  memset (&bgp_master, 0, sizeof (struct_bgp_master));

  bm = &bgp_master;
  bm->bgp = list_new ();
  bm->port = BGP_PORT_DEFAULT;
  bm->master__item = thread_master_create ();
